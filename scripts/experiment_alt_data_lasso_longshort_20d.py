"""Phase B v4 experiment driver — alt_data_screener_v4_2026_05_01.

Pre-registered as `alt_data_screener_v4_2026_05_01` in class
`alt_data_screener_search_2026_04_30` (n=4 -> Bonferroni |t| >= 2.50).
Pre-reg: `docs/research/preregistration/params_alt_data_screener_v4_2026_05_01.json`.

Same-class continuation of v3 FAIL (αt=−4.32 with 2/10 nonzero coefs and
+0.0260 holdout mean rank-IC). ONE variable changes vs v3: selection rule.
v3: top-30 long EW. v4: long top-decile EW MINUS short bottom-decile EW
with ex-ante SI <= 15% short-leg HTB filter (zen Objection #2 mitigation).

DIAGNOSTIC ONLY: capital deploy off-table regardless of holdout verdict
(burnt-holdout HARKing acknowledged in adversarial review).

10-feature whitelist (UNCHANGED from v3 / v4-v2):
...
All other architecture identical to v3 (settled variables).
"""
...
# Bonferroni n=4 in class -> |t| >= 2.50 per ledger threshold computation.
PASS_T = 2.50
PASS_SHARPE = 0.5
PASS_ALPHA_ANN = 0.03
PASS_MAX_DD = -0.35
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("docs/research/alt_data_screener_v4_phase_b.md"),
    )
    ap.add_argument(
        "--audit-out",
        type=Path,
        default=Path("docs/research/alt_data_screener_v4_audit.json"),
    )
    return ap
...
def main() -> int:
    ...
    logger.info(
        "v4 gates: in_cv_ir=%.3f (pass>=%.2f, mid>=%.2f) | holdout_mean_rank_ic=%.4f (pass>0) | n_holdout_asofs_with_ic=%d",
        in_cv_ir,
        PASS_IR,
        MID_IR,
        holdout_mean_ic,
...
def _write_audit(
    ...
    audit = {
        "id": "alt_data_screener_v4_2026_05_01",
...
def _write_report(
    ...
    lines = [
        ...
        "",
        "**Pre-registration:** `alt_data_screener_v4_2026_05_01` "
        "(class `alt_data_screener_search_2026_04_30`, n=4 -> Bonferroni |t|>=2.50)",
        "**Burnt-holdout caveat:** v4 is DIAGNOSTIC ONLY. Capital deploy "
        ...
        *regime_lines,
        ...
        "",
        "v4 in-class continuation: ONLY selection rule changed (top-30 long -> "
        "long top-decile EW MINUS short bottom-decile EW with SI<=15% short filter).",
    ]
    out_path.write_text("\n".join(lines))
