"""Phase B v5 long-only driver — alt_data_screener_v5_long_only_2026_05_01.

Pre-registered as `alt_data_screener_v5_long_only_2026_05_01` in class
`alt_data_screener_search_2026_04_30` (n=5 in-class, but threshold uses
PROGRAM-LEVEL n=10 -> |t| >= 2.81 per HARKing mitigation; stretch
|t| >= 3.2).
Pre-reg: `docs/research/preregistration/params_alt_data_screener_v5_long_only_2026_05_01.json`.

Path β auto-pivot from v10 (analyst_alt_data_v10_2026_05_01) Phase A
gate 2 survivorship FAIL. v10 pre-registered the pivot trigger
objectively (delisted/active event-rate ratio < 0.5 with z > 2 on
yfinance upgrades_downgrades) so the swap from γ to β is not post-hoc.

ONE variable changes vs v4: selection rule.
v4: long top-decile EW MINUS short bottom-decile EW with SI<=15% HTB filter.
v5: long top-decile EW only, vs SPY benchmark (NO short leg).

DIAGNOSTIC ONLY: capital deploy off-table regardless of holdout verdict
(burnt-holdout caveat persists; HARKing flag explicit per Kerr 1998 +
Simmons et al. 2011 — Path β designed against observed v4 long leg
+20.6%/y).

10-feature whitelist (UNCHANGED from v3 / v4):
...
All other architecture (Lasso, target rank-IC, train/holdout periods,
stride/holding, HAC=5, Lo-2002 Sharpe) identical to v4.
"""
...
# Program-level n=10 across all classes on this holdout -> |t| >= 2.81 (PRIMARY).
# Stretch threshold (Romano-Wolf m~50) -> |t| >= 3.2 (REPORTED alongside primary).
PASS_T = 2.81
STRETCH_T = 3.2
PASS_SHARPE = 0.5
PASS_ALPHA_ANN = 0.03
PASS_MAX_DD = -0.25  # tighter than v4 -0.35 because long-only excess vs SPY is less volatile
MID_T = 1.5
MID_SHARPE = 0.3
PASS_IR = 0.5  # reduced from v4 (1.0) since long-only IR distribution is naturally lower
MID_IR = 0.3
PASS_HOLDOUT_IC = 0.0
# Long-only is naturally market-exposed; β is descriptive (loose bound)
...
# ---------------------------------------------------------------------------
# v5 long-only decile portfolio builder (no short leg, vs SPY benchmark)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("docs/research/alt_data_screener_v5_long_only_phase_b.md"),
    )
    ap.add_argument(
        "--audit-out",
        type=Path,
        default=Path("docs/research/alt_data_screener_v5_long_only_audit.json"),
    )
    return ap
...
def main() -> int:
    ...
    # NB: cache key keyed on INPUT params only (universe set + window + stride/holding +
    # features version). NOT on realized calendar — yfinance refresh adds bars day-to-day,
    # which would needlessly invalidate the cache between sessions. Benchmark NOT in cache
    # key — features are benchmark-agnostic (benchmark only used to exclude from universe;
    # SPY/MDY both outside PIT universe).
    cache_key_payload = (
...
def _write_audit(
    ...
    audit = {
        "id": "alt_data_screener_v5_long_only_2026_05_01",
...
def _write_report(
    ...
    lines = [
        ...
        "",
        "**Pre-registration:** `alt_data_screener_v5_long_only_2026_05_01` "
