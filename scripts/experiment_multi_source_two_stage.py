"""Phase B experiment driver — multi_source_two_stage holdout reveal.
...
7. Fit per-regime Lasso w/ nested CV on train_pool only.
8. Predict scores on holdout features.
9. Top-30 equal-weight per asof; portfolio_return = 1d-forward mean (engine
    convention; matches existing Bonferroni cross-comparable scripts).
...
Pre-reg discipline: feature whitelist + lambda grid + train/holdout boundary
are FROZEN. Do NOT tune mid-experiment.
"""
...
def _load_pit_union(start: date, end: date) -> list[str]:
    """Union of PIT universe yamls overlapping [start, end] (per existing
    convention in scripts/experiment_momentum_lowvol_combo.py)."""
    union: set[str] = set()
...
# ---------------------------------------------------------------------------
# ADV filter — pre-reg requires ≥ $5M at scoring time


def _adv_at_asof(
    history_store: HistoryStore, ticker: str, asof: date, window: int = 20
) -> float | None:
    """Median dollar-volume over the last `window` bars <= asof. None if too short."""
...
def _apply_adv_filter(
    ...
    """Drop (asof, ticker) rows whose 20d median ADV is < adv_min_usd."""
...
# ---------------------------------------------------------------------------
# Backtest on holdout: top-N equal-weight, 1d-forward portfolio return
...
def _holdout_portfolio_returns(
    ...
    *,
    ...
    """Per holdout asof: take top_n by predicted score, equal-weight 1d forward.

    Returns
    -------
    (portfolio_returns_1d, bench_returns_1d, top_n_lists, holding_returns)
        portfolio_returns_1d : Series indexed by asof, values = mean 1d-forward
        bench_returns_1d     : same index, benchmark 1d forward
        top_n_lists          : per-asof tickers (used for turnover)
        holding_returns      : Series indexed by asof, mean holding_period forward
    """
    # Drop rows with NaN score (regimes that didn't fit, etc.)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("docs/research/multi_source_two_stage_phase_b.md"),
    )
    return ap
...
def _emit_result_line(
    ...
    """Format matches `audit_multi_phase._RESULT_LINE` regex."""
    if stats.get("n", 0) == 0:
        return
    logger.info(
...
def main() -> int:
    ...
    # 2. Insider parquet + delisting events (F4 fix)
    ...
    # 5. Build feature frame for full window
    ...
    # 9. Align train and fit per-regime Lasso
    ...
    logger.info("fitted regimes: %s", sorted(fits.keys()))
    if not fits:
        logger.error("no regime models fitted — abort")
        return 1

    # 10. Predict on holdout
    scores = predict_scores(fits, feat_holdout)
    ...
    # 12. Assess
    # Label intentionally omits phase_offset so audit_multi_phase aggregates
    # the 5 phases under a shared config key (per its `_config_key_from_line`
    # which splits on " | n=").
    label = f"HOLDOUT {args.holdout_start.year}-{args.holdout_end.year}"
    stats = _assess(
        port_rets, bench_rets, carhart, args.rebalance_stride, args.cost_half_spread_bps, top_lists
    )
    _emit_result_line(
        label,
        stats,
...
def _write_report(
    ...
    lines = [
        ...
        f"**Cost:** half-spread {args.cost_half_spread_bps:.0f} bps + 5 bps adverse selection (round-trip 2× one-way)",
        "",
        f"## Verdict: {verdict}",
        "",
    ...
    for regime, fit in fits.items():
        lines.append(
            f"| {regime} | {fit.n_train_obs} | {fit.chosen_alpha:.4g} | "
            f"{fit.n_nonzero_coefs} | {fit.cv_mean_mse:.4g} |"
        )
    lines += [
        "",
