"""event_drift v3 PEAD x quality experiment runner.
...
Pre-reg: docs/research/preregistration/ledger.json
  ``event_drift_v3_pead_quality_clean``.
"""
...
from alphalens.screeners.event_drift.accruals import SloanAccrualsStore
from alphalens.screeners.event_drift.announcement_dates import (
    AnnouncementDateProvider,
    EarningsAnnouncement,
)
from alphalens.screeners.event_drift.day1_filter import day1_return
from alphalens.screeners.event_drift.score_pead_quality import score_pead_quality
...
HOME = Path.home()
PIT_DIR = HOME / ".alphalens" / "pit_universe"
COMPANYFACTS_DIR = (
...
CIK_MAP_PATH = REPO_ROOT / "alphalens" / "data" / "alt_data" / "data" / "ticker_cik_map.yaml"

ETFS = ("SPY", "QQQ", "IWM", "MDY")


def _pit_universe_union(start_year: int, end_year: int) -> list[str]:
    """Return the union of PIT-universe tickers across yaml snapshots."""
    union: set[str] = set()
    for p in sorted(PIT_DIR.glob("*.yaml")):
        try:
            yr = int(p.stem.split("-")[0])
        except ValueError:
            continue
        ...
        try:
            ...
            logger.warning("YAML error in %s: %s", p, exc)
            continue
...
class _HistoryCalendar:
    ...
    def snap_to_trading_day(self, d: date) -> date:
        if d in self._date_set:
            return d
        # Find smallest trading day >= d (advance on weekends/holidays)
        idx = self._search_next(d)
        if idx is None:
            return d  # past end of calendar; return as-is
    ...
    def next_trading_day(self, d: date) -> date:
        ...
        if snapped > d:
            return snapped
        # snapped == d (it was a trading day) -> advance
...
def _build_announcement_index(
    ...
    """Pre-fetch all announcements per ticker once (no asof filter)."""
...
def _make_announcement_lookup(raw_index: Mapping[str, list[EarningsAnnouncement]], asof: date):
    """Filter announcements to filed_date <= asof for PIT correctness."""
...
def _make_day1_lookup(history: HistoryStore):
    """Return (ticker, market_day) -> close-to-close return at market_day."""
...
def _adv_filter(
    ...
    """Drop tickers whose trailing ``window_days`` ADV $-volume is below ``adv_min_usd``."""
    keep: list[str] = []
...
def _setup_stores(universe: list[str]) -> tuple:
    """Load all the stores + history + announcement index."""
...
def _run_breadth_audit(args: argparse.Namespace) -> int:
    """Fast breadth audit: pre-build all event windows once, then filter per asof.

    Avoids the O(asofs x tickers x announcements) explosion of the naive
    per-asof scorer call. Day-1 returns and SUE/accruals are computed once
    per (ticker, period_end) and stored on the EventWindow.
    """
    ...
    universe = _pit_universe_union(start.year - 5, end.year)
    universe_with_history = [t for t in universe if (PRICES_DIR / f"{t}.parquet").exists()]
    print(
        f"PIT-yaml union: {len(universe)}; with cached OHLCV: {len(universe_with_history)}",
        flush=True,
    )
    ...
    # Pre-build ALL event windows ONCE. This is the big cost; later steps
    # are filters over the pre-built list.
    t1b = time.time()
    ...
    for i, ticker in enumerate(universe_with_history):
        ...
        if not anns:
            continue
        for ann in anns:
            # Only look at announcements within (or just before) the audit window
            # plus 60d buffer so the 90d cohort is well-formed at start
    ...
    # Apply single-active-window invariant ONCE
    t1c = time.time()
    ...
    # Pre-compute Day-1 returns for each window's market_day (asof-independent)
    t1d = time.time()
    ...
    # Pre-filter windows that pass Day-1 sign confirmation (asof-independent)
    ...
    for i, asof in enumerate(asofs):
        # Active windows at asof
        active = windows_active_on(survivors, asof)
        if not active:
            breadths.append(0)
            continue

        # Cohort = windows with market_day in [asof - 90d, asof]
        ...
        # ADV filter — keep those with sufficient $-volume at asof
...
def _run_smoke(args: argparse.Namespace) -> int:
    ...
    universe = _pit_universe_union(asof.year - 2, asof.year + 1)
    universe_with_history = [t for t in universe if (PRICES_DIR / f"{t}.parquet").exists()]
    if args.max_tickers:
        universe_with_history = universe_with_history[: args.max_tickers]
