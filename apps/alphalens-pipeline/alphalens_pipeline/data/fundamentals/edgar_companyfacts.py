...
def _evict_to_capacity(cache: dict, max_size: int) -> int:
    ...
    Duplicate of the same helper in ``alphalens_pipeline.data.alt_data.sec_edgar_client``;
    kept independent to avoid coupling ``alphalens_pipeline.data.fundamentals`` to
    ``alphalens_pipeline.data.alt_data``. Returns the number of entries actually evicted
