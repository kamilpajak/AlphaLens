...
PIT storage, scorer library, and the daily / cron jobs that run as
systemd-user units on the VPS.

## Sub-packages

| Path | Status | Notes |
|------|--------|-------|
| `alphalens_pipeline/edgar_detector/` | ACTIVE | Layer 1 SEC EDGAR detection — VPS systemd `detect`-only |
| `alphalens_pipeline/thematic/` | ACTIVE | Phase A-E daily pipeline — VPS systemd |
| `alphalens_pipeline/literature_scanner/` | ACTIVE | Monthly + weekly Perplexity scan — VPS systemd |
