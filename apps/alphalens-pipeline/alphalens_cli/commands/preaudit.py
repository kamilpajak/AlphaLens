...
1. Coverage — :func:`alphalens_research.preaudit.coverage.check_all_deps` peeks
   the strategy's data deps under ``~/.alphalens/`` for existence +
   date-range coverage. Cheap (~1 s). Fails before subprocess spawn.
2. Smoke — :func:`alphalens_research.preaudit.runner.run_smoke` executes the
...
# into the import graph. Keeps the pipeline → research dependency direction
# clean at workspace level; see CLAUDE.md "Lazy CLI imports" + alphalens-
# pipeline's README for the rationale.

# CLI surface duplicate of ``alphalens_research.preaudit.runner.DEFAULT_SMOKE_TIMEOUT_S``.
# typer.Option evaluates its default at function-definition time (module
# import), so we can't lazy-import it. Sync with the research-side value
# by hand if it ever changes — tests/test_preaudit_cli_default_in_sync.py
# pins the equality.
_DEFAULT_SMOKE_TIMEOUT_S = 600
...
def _resolve_profile(strategy: str):
    ...
    if profile is None:
        typer.echo(
            ...
            f"Add one to alphalens_research/preaudit/profiles.py::SMOKE_PROFILES "
...
def _run_smoke_stage(strategy: str, profile, timeout_s: int) -> None:
    ...
    from alphalens_research.preaudit.profiles import SmokeStatus
...
def preaudit_command(
    ...
    timeout_s: int = typer.Option(
        ...
        "--timeout-s",
        help=f"Wall-clock budget for the smoke subprocess (default: {_DEFAULT_SMOKE_TIMEOUT_S}s).",
    ),
