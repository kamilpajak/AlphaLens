...
`alphalens_research.overlays` is a sibling layer to `alphalens_research.gates`
...
class VolTargetScalingTests(unittest.TestCase):
    ...
    def test_zero_or_negative_target_vol_raises(self):
        from alphalens_research.overlays import VolTargeter
    ...
    def test_zero_realized_vol_logs_warning_not_silent_one(self):
        ...
        with self.assertLogs("alphalens_research.overlays.vol_target", level=logging.WARNING) as cm:
            scaled = apply_vol_target(rets, targeter)
...
class RiskOverlayPackageStatusTest(unittest.TestCase):
    def test_package_declares_research_only_status(self):
        ...
        2026-04-30. Package mirrors `alphalens_research.gates` and `macro/`."""
        import alphalens_research.overlays as pkg
