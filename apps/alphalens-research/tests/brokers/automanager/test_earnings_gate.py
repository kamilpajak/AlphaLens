"""The earnings-window gate refuses to place entries that would rest across a
...
geometry at its most wrong. The gate refuses placement when the ticker's next
...
from alphalens_pipeline.brokers.automanager import earnings_gate
...
class TestEarningsWindowRefusal(unittest.TestCase):
    def setUp(self):
        earnings_gate._clear_lookup_cache_for_tests()
        # Hermetic: the opt-out env must not leak in from the host environment.
        self._env = mock.patch.dict(os.environ, {}, clear=False)
        self._env.start()
        os.environ.pop(earnings_gate.EARNINGS_GATE_OPT_OUT_ENV, None)

    def tearDown(self):
        self._env.stop()

    def test_earnings_inside_window_refuses_with_the_date(self):
        reason = earnings_gate.earnings_window_refusal(
    ...
    def test_earnings_exactly_at_window_end_refuses(self):
        ...
        window_end = earnings_gate._window_end(_TODAY, 7)
        reason = earnings_gate.earnings_window_refusal(
    ...
    def test_lookup_exception_fails_open(self):
        ...
        reason = earnings_gate.earnings_window_refusal("WK", ttl_days=7, today=_TODAY, lookup=_boom)
        self.assertIsNone(reason)

    def test_env_opt_out_skips_the_gate_without_lookup(self):
        lookup = _lookup_returning(dt.date(2026, 7, 30))
        with mock.patch.dict(os.environ, {earnings_gate.EARNINGS_GATE_OPT_OUT_ENV: "1"}):
            reason = earnings_gate.earnings_window_refusal(
    ...
    def test_default_lookup_is_cached_per_ticker_and_day(self):
        # The drain retries a refused pick EVERY ~45s tick; without a per-day
        # cache each retry would be a fresh yfinance call.
        with mock.patch.object(
            earnings_gate, "_fetch_next_earnings", return_value=dt.date(2026, 8, 4)
        ) as fetch:
            first = earnings_gate.earnings_window_refusal("KTOS", ttl_days=7, today=_TODAY)
            second = earnings_gate.earnings_window_refusal("KTOS", ttl_days=7, today=_TODAY)
