...
class TestMechanicalSalienceCandidates:
    def test_equal_weight_membership_theme_filtered_with_counts(self, tmp_path: Path):
        ...
        assert by_ticker == {"AAPL": 2, "NVDA": 1}
        assert "MSFT" not in by_ticker

    def test_respects_lookback_window(self, tmp_path: Path):
    ...
    def test_tickers_uppercased(self, tmp_path: Path):
        ev = tmp_path / "events"
    ...
    def test_empty_when_no_events(self, tmp_path: Path):
        ev = tmp_path / "events"
        ev.mkdir()
        out = ps.mechanical_salience_candidates(
            "ai", dt.date(2026, 6, 10), events_dir=ev, lookback_days=30
...
class TestBuildShadowFrame:
    def test_both_sources_present_with_correct_columns(self, tmp_path: Path):
        ev = tmp_path / "events"
        _write_events(ev, "2026-06-10", [{"primary_entities": ["AAPL", "NVDA"], "themes": ["ai"]}])
        llm = [
        ...
        assert set(frame["source"]) == {"llm", "mechanical"}
        ...
        assert set(llm_rows["ticker"]) == {"TSLA", "AAPL"}
        assert set(mech_rows["ticker"]) == {"AAPL", "NVDA"}
        assert llm_rows["llm_confidence"].notna().all()
        assert llm_rows["mech_article_count"].isna().all()
        assert mech_rows["mech_article_count"].notna().all()
        assert mech_rows["llm_confidence"].isna().all()
        # Poolability keys stamped on every row.
        assert (frame["mapper_config_version"] == "cfg-x").all()
        assert (frame["mech_rule_version"] == ps.MECH_RULE_VERSION).all()
        assert (frame["proposal_shadow_version"] == ps.PROPOSAL_SHADOW_VERSION).all()
        assert (frame["brief_date"] == dt.date(2026, 6, 10)).all()

    def test_llm_only_theme_still_logs_llm_rows_when_no_news(self, tmp_path: Path):
        ev = tmp_path / "events"
        ev.mkdir()
        ...
        assert list(frame["source"]) == ["llm"]
        assert list(frame["ticker"]) == ["TSLA"]


class TestWriteProposalShadow:
    def test_writes_parquet_at_dated_path_and_roundtrips(self, tmp_path: Path):
        ev = tmp_path / "events"
        _write_events(ev, "2026-06-10", [{"primary_entities": ["AAPL"], "themes": ["ai"]}])
        ...
        assert path == out_dir / "2026-06-10.parquet"
        back = pd.read_parquet(path)
        assert set(back["source"]) == {"llm", "mechanical"}

    def test_empty_llm_and_no_news_writes_nothing_returns_none(self, tmp_path: Path):
        ev = tmp_path / "events"
        ev.mkdir()
        out_dir = tmp_path / "shadow"
        path = ps.write_proposal_shadow(
        ...
        assert path is None
        assert not (out_dir / "2026-06-10.parquet").exists()
