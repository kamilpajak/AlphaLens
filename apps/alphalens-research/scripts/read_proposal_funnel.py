...
def wilson(k: int, n: int) -> tuple[float, float]:
    """95% Wilson interval — well behaved at 0 and at n, unlike the normal one."""
    if n == 0:
...
def _permutation_homogeneity_p(
    ...
    labels = np.concatenate([np.ones(n_big), np.zeros(int(sizes.sum()) - n_big)])
...
def _prompt_sha(config_version: str) -> str:
    ...
    try:
        return str(json.loads(config_version).get("prompt_sha", "absent"))
...
def build_report(df: pd.DataFrame) -> dict:
    ...
    report["integrity"] = {
        ...
        "mcap_null_but_resolved": int(
            ((df["bracket_verdict"] != NO_MCAP) & df["market_cap"].isna()).sum()
        ),
    ...
    resolved = df[df["bracket_verdict"] != NO_MCAP]

    # ---- PRIMARY ---------------------------------------------------------
    n_res = len(resolved)
    k_big = int((resolved["bracket_verdict"] == TOO_BIG).sum())
    lo, hi = wilson(k_big, n_res)
    report["primary"] = {
    ...
    for day, g in df.groupby("_file_asof", sort=True):
        g_res = g[g["bracket_verdict"] != NO_MCAP]
        k = int((g_res["bracket_verdict"] == TOO_BIG).sum())
        n = len(g_res)
        d_lo, d_hi = wilson(k, n)
        per_day.append(
            {
                "asof": day,
                "proposals": len(g),
    ...
    eligible = [d for d in per_day if d["resolved"] >= MIN_RESOLVED_FOR_TEST]
    table = [
        [
            d["counts"][TOO_BIG],
            d["resolved"] - d["counts"][TOO_BIG],
        ]
        for d in eligible
    ]
    chi2 = pval = float("nan")
    ...
    pooled_share = float(report["primary"]["share"])
    ...
    report["homogeneity"] = {
        ...
        "reliable": bool(min_expected >= 5) if not math.isnan(min_expected) else False,
        # The contract says a chi-square with a small expected cell is unreliable
        # and the range is used instead. The range alone cannot separate real
        # day-to-day movement from the sampling noise of a 9-proposal day, so the
        # declared remedy for exactly that condition is run too: the same
        # statistic, its null distribution obtained by reshuffling proposals
        # across days. Same estimand, no distributional assumption.
        "permutation_p": perm_p,
        "observed_range": [min(shares), max(shares)] if shares else [float("nan")] * 2,
        "max_abs_deviation_pp": 100 * worst,
        "within_band": bool(100 * worst <= STABILITY_BAND_PP),
        "band_pp": STABILITY_BAND_PP,
    }

    # ---- SECONDARY: full cap distribution --------------------------------
    caps = resolved["market_cap"].dropna().astype(float)
    report["cap_distribution"] = {
        "n": len(caps),
        "min": float(caps.min()),
        "max": float(caps.max()),
        ...
        "log10_histogram": {
            f"1e{b}": int(((caps >= 10.0**b) & (caps < 10.0 ** (b + 1))).sum())
            for b in range(6, 13)
        },
    }
...
def render(r: dict) -> str:
    ...
    out.append(f"mapper_config_versions: {len(ig['mapper_config_versions'])}")
    ...
    out.append(
        ...
        f"(Wilson 95% {pct(p['wilson95'][0])} .. {pct(p['wilson95'][1])})"
    )
    ...
    for d in r["per_day"]:
        ...
        out.append(
            ...
            f"{pct(d['too_big_share'])} ({pct(d['wilson95'][0])}..{pct(d['wilson95'][1])}){mark}"
        )

    h = r["homogeneity"]
    out.append("")
    out.append("=== HOMOGENEITY (contract test) ===")
    out.append(
        f"chi2={h['chi2']:.2f} p={h['p']:.4f} over {h['days_tested']} days; "
        f"min expected cell {h['min_expected_cell']:.1f} -> "
        f"{'reliable' if h['reliable'] else 'NOT reliable, use the permutation p'}"
    )
    out.append(f"permutation p (same statistic, 20k reshuffles) = {h['permutation_p']:.4f}")
    out.append(
        f"observed range {pct(h['observed_range'][0])} .. {pct(h['observed_range'][1])}; "
        f"max deviation from pooled {h['max_abs_deviation_pp']:.1f} pp "
        f"(band {h['band_pp']:.0f} pp) -> {'STABLE' if h['within_band'] else 'VARIES'}"
    )
    ...
    out.append(
        "  histogram: " + "  ".join(f"{k}:{v}" for k, v in c["log10_histogram"].items() if v)
    )
