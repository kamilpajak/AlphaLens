"""v9D — retrospective pre-2018 fresh-OOS replication.

Pre-registered as ``v9d_retrospective_pre_2018_2026_05_05`` in signal class
``retrospective_replication_pre_2018``. Locks frozen v9D scorer
(``score_cross_sectional_residual``), three universe variants
(U1/U2/U3), three sub-periods, and five phase offsets → 45 cells total.

Per-cell artifacts:
- Carhart 4F αt + alpha_pct + alpha_pct_se (primary)
- Sharpe net, IR, MaxDD, turnover
- Raw long-only top-decile return series (for pooled bootstrap downstream)

The driver is intentionally narrow: ONE cell per invocation. Bash loop /
GPU pod orchestrator runs all 45 combinations. Multi-phase + multi-universe
+ multi-sub-period verdict synthesis lives in Phase 4 aggregator.
"""
...
from alphalens_research.retrospective_audit.universe_loaders import (
...
# Sub-period boundaries — locked in pre-reg JSON. Encoded here for offline
# selection so the driver does not depend on a JSON parse at run time.
...
def _benchmark_calendar(
    ...
    """Calendar of stride-spaced rebalance asof dates from benchmark cache.

    Uses ``close.notna()`` as the trading-day filter rather than ``ivp30`` —
    iVol's MDY ETF cache has full ``close`` history (2007-2026) but ``ivp30``
    coverage stops in 2009 (the index-ETF doesn't carry the per-ticker IV
    percentile after that vintage). The legacy v10 driver hits the
    2018-2026 holdout where MDY ``ivp30`` exists; this driver targets
    pre-2018 and must use ``close``."""
...
def _load_universe(variant: str, asof_dates: list[date]) -> list[str]:
    """Resolve U1/U2/U3 to a single union of candidate tickers across asofs.

    For U1 (legacy yaml) the loader is asof-agnostic — returns the static
    union. For U2/U3 we take the union over all asofs within the sub-period
    so the feature joiner can run on a stable input frame; asof-by-asof
    membership is handled downstream via inventory cutoff."""
    if variant == "U1":
    ...
    if variant == "U3":
        seen = set()
        for asof in asof_dates:
            seen.update(pit_union_nber_rebuild(asof, on_missing_shares="exclude", extra_etfs=ETFS))
        return sorted(seen)
    raise ValueError(f"Unknown universe variant {variant!r} (expected U1/U2/U3)")
...
def _portfolio_returns(
    ...
    *,
    ...
    """Long-only top-decile per-rebalance returns + ranked-tickers history.

    Returns ``(rets, asof_idx, decile_sizes, top_decile_history)`` where
    ``top_decile_history`` is a list of per-asof sets of tickers (used for
...
def _turnover(holdings_history: list[set]) -> float:
    """Mean per-rebalance one-sided turnover ratio (0 = static, 1 = full
    replacement)."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        choices=["U1", "U2", "U3"],
        required=True,
        help="Universe variant. U1=legacy yaml, U2=ivol cache, U3=NBER rebuild.",
    )
    ap.add_argument(
        "--sub-period",
        choices=list(SUB_PERIODS.keys()),
        required=True,
        help="Pre-registered sub-period name.",
    )
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=REPO_ROOT / "docs" / "research" / "v9d_retrospective_pre_2018",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    stats = _stats(
        ...
        label=f"v9D {args.universe} {args.sub_period} p{args.phase_offset}",
    )
