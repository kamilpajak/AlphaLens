...
def _evict_to_capacity(cache: dict, max_size: int) -> int:
    ...
    Duplicate of the same helper in ``alphalens_research.data.alt_data.sec_edgar_client``;
    kept independent to avoid coupling ``alphalens_research.data.fundamentals`` to
    ``alphalens_research.data.alt_data``. Returns the number of entries actually evicted
