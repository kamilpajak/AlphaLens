...
The refreshed YAML is compatible with :class:`alphalens_research.data.alt_data.ticker_cik_map.TickerCikMap`,
so a production workflow is::

    client = SecEdgarClient(user_agent="AlphaLens research@example.com")
    refresh_ticker_cik_map(
        client, Path("alphalens_research/data/alt_data/data/ticker_cik_map.yaml")
    )
