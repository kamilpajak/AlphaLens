...
callable, so Layer 2b themed (momentum | early-stage scorers), the archived
Lean rule-based scorer, or any future scorer can all be replayed through
the same harness. Loading OHLCV
histories is the caller's responsibility — e.g.
`alphalens_research.archive.screeners.lean.lean_csv_loader.load_lean_histories` for the Lean
zip-CSV store.
"""
