		...
		<div class="relative grid grid-cols-2 gap-x-6 gap-y-3 mb-4 text-[10px] uppercase tracking-widest">
			<div>
				...
				<div class="text-fg text-base font-bold normal-case">
					{setup.suggested_size_pct != null ? `${setup.suggested_size_pct}% of book` : '—'}
				</div>
			</div>
			<div>
			...
			<div class="flex flex-col gap-1">
				{#each setup.entry_tiers as tier, i}
					...
					>
						...
						<span class="hidden sm:block text-fg-muted whitespace-nowrap">-{tier.atr_distance.toFixed(1)} ATR</span>
			...
			<div class="flex flex-col gap-1">
				{#each setup.tp_tranches as tp, i}
					...
					>
						...
						<span class="hidden sm:block text-fg-muted whitespace-nowrap">{tp.r_multiple.toFixed(1)}R</span>
