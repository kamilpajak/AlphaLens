...
class TestNormalizeInsiders(unittest.TestCase):
    ...
    def test_sec_form4_code_P_maps_to_buy(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["P"], "Shares": [1000], "PricePerShare": [500.0],
        })
    ...
    def test_sec_form4_code_S_maps_to_sell(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["S"], "Shares": [1000], "PricePerShare": [500.0],
        })
        out = normalize_insiders(raw)
    ...
    def test_verbose_purchase_maps_to_buy(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["Purchase"], "Shares": [1000], "PricePerShare": [500.0],
        })
        out = normalize_insiders(raw)
        self.assertEqual(out["transaction"].iloc[0], "A")
    ...
    def test_verbose_sale_maps_to_sell(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["Sale"], "Shares": [1000], "PricePerShare": [500.0],
        })
        out = normalize_insiders(raw)
        self.assertEqual(out["transaction"].iloc[0], "D")
    ...
    def test_acquired_maps_to_buy(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["Acquired"], "Shares": [1000], "PricePerShare": [500.0],
        })
        out = normalize_insiders(raw)
        self.assertEqual(out["transaction"].iloc[0], "A")
    ...
    def test_code_A_maps_to_buy(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["A"], "Shares": [1000], "PricePerShare": [500.0],
        })
        out = normalize_insiders(raw)
        self.assertEqual(out["transaction"].iloc[0], "A")
    ...
    def test_unknown_code_defaults_to_sell(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["F"], "Shares": [1000], "PricePerShare": [500.0],
        })
        out = normalize_insiders(raw)
        self.assertEqual(out["transaction"].iloc[0], "D")
    ...
    def test_value_equals_shares_times_price(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "Date": ["2024-01-10"], "Name": ["CEO"],
            "Transaction": ["A"], "Shares": [1500], "PricePerShare": [200.0],
        })
        out = normalize_insiders(raw)
...
class TestNormalizeCongress(unittest.TestCase):
    ...
    def test_partial_sale_canonicalised_to_sale(self):
        ...
        raw = pd.DataFrame({
            "Ticker": ["NVDA"], "TransactionDate": ["2024-01-10"],
            "Representative": ["X"], "Transaction": ["Sale (Partial)"],
            "Range": ["$1,001 - $15,000"],
        })
        out = normalize_congress(raw)
