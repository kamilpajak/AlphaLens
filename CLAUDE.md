...
**Web — Storybook is the `apps/web` component catalog + spec** — Storybook (SB 10, `@storybook/addon-svelte-csf`) is dev-only (tree-shaken from the prod CF Pages build; zero runtime cost). Doctrine:
- **Every new / changed `apps/web` component ships a `.stories.svelte`** next to it, covering the meaningful STATES (default / empty / loading / error / edge) — one state per `<Story>`, not a kitchen-sink.
- **Check the catalog before building new UI** — reuse existing primitives (`StatusPill`, `LedgerRow`, `Disclosure`, `SectionPanel`, `DetailField`, `EvidenceLink`, `LedgerFilterBar`, the tooltip family) instead of reinventing; read the component source for its real props — never invent props.
- **Bind REAL fixtures, never invented numbers** — data-bearing stories use `apps/web/tests/fixtures/api-mock/*` (`days/2026-05-18.json`, `edge-summary.json`, `edge-outcomes.json`) or the component's own exported enums. Fake-but-plausible data is a defect (a common AI-authored-story failure mode).
- **`play` is a PROP, not a snippet** — `play={async ({ canvas }) => …}`, NEVER `{#snippet play}` (the snippet form silently never runs as the play fn). Use the `canvasElement` play-arg (not `canvas`) for raw-DOM queries (e.g. `.querySelector('math')`). Alias a duplicate value+type default import (`import type FooInstance from './Foo.svelte'`) for a `bind:this` `$state<FooInstance | null>` ref.
- **Gate — run from `apps/web`, NOT the workspace root:** `pnpm run check` + `pnpm run build-storybook` must both pass. There is **no Storybook test-runner in CI**, so `play()` assertions are NON-gating — they execute only when a story is opened in `pnpm run storybook`; verify there when a story's interaction matters. a11y `tabindex` warnings on tooltip triggers carry `<!-- svelte-ignore a11y_no_noninteractive_tabindex -->`.
- Full-coverage catalog landed PR #777 (2026-07-08, authored via Workflow). See memory `project_storybook_catalog_expansion_2026_07_08`.
...
Sites with an injected client (edgar_detector `SECEdgarSource`, `CIKLoader`; thematic mapper/extractor/generator; thematic press verification + news ingest; `PolygonShortInterestClient` domain wrapper) keep DI; module-level helpers call the respective `get_default_*_client()`. The same doctrine now also covers **yfinance** (`yfinance_client.py`), **Perplexity** (`literature_scanner/perplexity_client.py`), **FRED** (`data/macro/fred_client.py`), **GDELT** (`gdelt_client.py`), **Telegram** alerts (`telegram_client.py`), and **iShares** holdings CSV (`ishares_client.py`). Env vars: `SEC_EDGAR_USER_AGENT`, `ALPHA_VANTAGE_API_KEY`, `OPENROUTER_API_KEY`, `POLYGON_API_KEY`, `PERPLEXITY_API_KEY`, `FRED_API_KEY`, `TELEGRAM_BOT_TOKEN` (GDELT + iShares are keyless). Enforcement: one `apps/alphalens-research/tests/test_no_raw_<vendor>_http.py` per vendor (`sec`, `av`, `openrouter`, `polygon`, `yfinance`, `perplexity`, `fred`, `gdelt`, `telegram`, `ishares`) — each with a positive-control case so the regex / URL-list cannot rot to empty silently. `test_no_raw_yfinance_http.py` keeps a deferred-migration allowlist (`yfinance_cache.py`, the RESEARCH_ONLY prescreener, 4 Mac-only research scripts) with an anti-rot check that forces removal once migrated. NOT given a client (by design): RSS/`feedparser` (multi-host, no single quota'd vendor — single `_parse_feed` seam) and the display-only generic-publisher og:title fetch in `canonical_title.py`. Design memos: `docs/research/{sec_edgar,alphavantage,gemini,polygon}_client_consolidation_2026_05_*.md` (the gemini memo records the now-completed consolidation + removal).
...
- **GDELT theme YAML — multi-word quoted phrases only** — `alphalens_pipeline/thematic/config/gdelt_themes.yaml` queries must use multi-word phrases in quotes (e.g. `"CUDA toolkit"`, NOT `"CUDA"`). GDELT DOC API rejects single-word quoted tokens with `HTTP 200 + "The specified phrase is too short."` — the canonical `gdelt_client._http_get_json` raises this immediately as `GdeltQueryError` (no retry, no rate-limit burn). Static lint in `apps/alphalens-research/tests/thematic/test_gdelt.py::TestGdeltThemesYamlWellFormed` guards against regression. Live smoke per-bucket: `GDELT_LIVE_TEST=1 .venv/bin/python -m unittest tests.thematic.test_gdelt_live -v` (~90s wall, opt-in).
- **Catalyst source-gate — state-media blocklist** (`catalyst_resolver._filter_entityless_events`, PR #634) — a per-ticker catalyst (the brief `source_event`) is the latest theme-tagged news event. **Entity-less** events (LLM `primary_entities=[]`, i.e. the article names no company) are eligible **by default** and dropped ONLY when the GDELT `domain` is in `state_media_domain_blocklist` OR the GDELT `sourcecountry` is in `state_media_countries` — both in `alphalens_pipeline/thematic/config/catalyst_noise_filters.yaml`. **Entity-RICH events always pass** regardless of language/source, so this never affects foreign-language company news (Chinese NVDA, Korean Intel). **No language arm** — the discriminator is the SOURCE, not the language (legit German/French/Polish/Taiwan entity-less news is kept; only state media is dropped). `domain`/`sourcecountry` live in the news `extra` JSON and exist only for GDELT (RSS/Polygon/EDGAR entity-less always pass). **To block a new propaganda source:** add its bare host to `state_media_domain_blocklist` (host-exact or registrable-suffix match; `rt.com` blocks `news.rt.com` but not `report.com`) or its GDELT country name to `state_media_countries`, each commented with its authority. China/Russia GDELT `sourcecountry` strings are live-verified; Iran/North Korea/Belarus are fail-open backstops (a spelling mismatch silently keeps the event, never over-blocks). Why entity-less ≠ noise: ~97% of entity-less events are reputable English journalism (MarketWatch/TechCrunch/FT); the real noise is ~10-25 GDELT Chinese state-media articles (the original `voc.com.cn` → BAH/PSN/AVAV false catalyst). Tests: `apps/alphalens-research/tests/thematic/mapping/test_catalyst_resolver.py` (resolver-level) + `tests/thematic/test_catalyst_gate_map_themes_integration.py` (the `map_themes` skip cascade, #635). Design: `docs/research/catalyst_noise_discriminator_design_2026_06_18.md`. NOTE: the related Chinese `source_event_title` *display* (a real foreign-language headline surfaced on the card) is a SEPARATE pre-existing issue — this gate fixes catalyst SELECTION, not the title text.
...
| `alphalens-grouped-daily-topup.{service,timer}` | daily oneshot at 01:30 UTC (`Type=oneshot` + `OnCalendar=*-*-* 01:30:00 UTC` + `Persistent=true`) | `.venv/bin/python apps/alphalens-research/scripts/backfill_grouped_daily_history.py --topup` (host venv) — appends the latest missing session(s) to the persistent split-adjusted (`adjusted=true`) grouped-daily store that feeds the O'Neil **R** (relative-strength) term. Self-sizing from newest-on-disk (capped `--max-catchup-days` 400); one whole-market grouped-daily call per missing session; entitlement-cliff (NOT_AUTHORIZED past the free-tier ~21–24mo window) stops cleanly. DISTINCT from the population-monitor's `adjusted=false` cache — the two MUST NOT be merged (RS needs split-adjusted closes; the monitor needs raw closes for minute-bar/ladder matching). One-time seed via the same script without `--topup` (`--sessions N`). | `~/.alphalens/grouped_daily_history/<YYYY-MM-DD>.parquet` (one whole-market snapshot per session; read disk-only at the thematic `score` stage via `rs_history.rs_percentile`) | ~minutes steady-state (1 grouped-daily call); one-time seed ~88min for ~440 sessions at Polygon free 5 req/min | LIVE 2026-06-14 (PR-A #566 + PR-B #567). Needs `POLYGON_API_KEY`. Monitoring: `AlphalensJobStale`@48h + `MetricMissing`. Design: `docs/research/oneil_r_reactivation_design_2026_06_14.md`. |
| `alphalens-thematic-build.{service,timer}` | 6× daily oneshot at HH:30 UTC (00/04/08/12/16/20) with `RandomizedDelaySec=5min` + `Persistent=true` + `TimeoutStartSec=75min` (bumped 45→75min #582 after the grown pipeline + a yfinance DNS storm pushed two slots past the old 45min mid-`experts enrich`, killing them before `rebuild-cache`), wrapping `docker run --rm alphalens-pipeline` + `compose run --rm rebuild-cache` (Django stack) | `alphalens thematic {ingest,extract,map-themes,score,brief}` (ingest passes `--force`; the `score` stage stamps the Buffett + O'Neil numerics + the panel `expert_spread`/`panel_config_version` scalars + the 16 `options_*` telemetry cols per PR #772 — yfinance chain snapshot, stamped only by slots inside the post-close window for the asof session (in practice 00:30/04:30/08:30 UTC), first success freezes, failed fetches retry across slots) + `alphalens experts migrate-qual-cache` + `alphalens experts enrich <yesterday> --all --scuttlebutt` (best-effort, in `run_thematic_day.sh` AFTER `brief`, BEFORE rebuild-cache; renamed from `buffett qual-enrich` in PR-2/#554) + `manage.py rebuild_briefs_cache` | `~/.alphalens/thematic_briefs/` (the `score` stage stamps 6 `buffett_*` quant + 8 `oneil_*` + 2 panel cols + 16 `options_*` telemetry cols; `experts enrich` stamps the Buffett qual cols in place) + `~/.alphalens/buffett_qual/<date>/<TICKER>{.sb}.json` LLM-result cache + Postgres `briefs`/`days_meta` tables | ~12-20 min (+~6 min first run per date for eager Buffett qual; cached per (date,ticker,scuttlebutt) so the 6×/day reruns skip it) | LIVE 2026-05-30 — 6×/day cadence per PR-F (epic #295 #300) maps onto global exchange rotation (XTKS/XHKG/XSHG/XWAR/XNYS); feeds CF Pages SPA via Django API. **Expert-panel epic (#541, PR-0..9):** the score stage runs Buffett (value/quality) + O'Neil (momentum, numeric-only) + the `disagreement.py` `expert_spread` scalar; Django assembles the `expert_assessments` blob `{buffett, oneil, panel}` (no migration, reuses JSONField 0011); `experts enrich --all` adds the eager Buffett qual layer (moat/trend/candor/understandable + rationale from 10-K, `--scuttlebutt` web-context via Perplexity). The card shows a `buffett NN/100` chip + a tone-neutral `panel N lenses` coverage chip + a generalized `expert.panel` deep-read drawer (Buffett qual pillars + O'Neil numerics + the disagreement headline/dot-lane); all display-only / out of the brief sort. The cheap Buffett durability facts also feed the brief bear case (#538). Needs `OPENROUTER_API_KEY` + `PERPLEXITY_API_KEY` + `SEC_EDGAR_USER_AGENT` (already passed into the container). Activates on the next VPS-local `alphalens-pipeline:latest` image rebuild (script is baked in). |
...
- Runtime data (outside repo, survives git ops):
  ...
  - `~/.alphalens/grouped_daily_history/<date>.parquet` — persistent split-adjusted (`adjusted=true`) whole-market daily-close history feeding the O'Neil R term (read disk-only at the `score` stage via `rs_history.rs_percentile`); kept fresh by `alphalens-grouped-daily-topup`. SEPARATE from the monitor's `adjusted=false` grouped cache
  - `~/.alphalens/thematic_briefs/` — daily thematic pipeline parquets (consumed by Django briefs ingest; carry the `buffett_*` + `oneil_*` + panel `expert_spread`/`panel_config_version` cols that Django assembles into the `expert_assessments` `{buffett, oneil, panel}` blob, plus the 16 `options_*` telemetry cols — parquet-only SoT, Django ingest drops them by design; first-look vs EDGE at N>=30 with `chain_quality=OK` ~2026-09, design memo `docs/research/options_telemetry_design_2026_07_07.md`)
