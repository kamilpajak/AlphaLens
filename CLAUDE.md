...
- **API (`apps/alphalens-django/`)** — Docker image pushed to `ghcr.io/kamilpajak/alphalens-django` per CI workflow `.github/workflows/django-image.yml`. VPS pulls + runs the image (Phase 3 splits compose into pull-only canonical + local-dev override). API reached cross-origin from CF Pages — `CORS_ALLOWED_ORIGINS` in Django prod env must list the Pages URL.
- **Origin** — Cloudflare Tunnel from VPS to `api.<domain>` mapped to `localhost:8000` (Django gunicorn behind Tunnel). `auth_cf` middleware validates `CF-Access-Jwt-Assertion` headers. **Auth path for SPA → API is undecided** (Phase 3 blocker): path A = same-domain cookies (SPA at `app.<domain>`, browser sends `CF_Authorization` cookie cross-origin, `CORS_ALLOW_CREDENTIALS=True`); path B = Pages Function edge proxy holding a Service Token as secret env var. Service Tokens MUST NOT live in browser JS. CF Access app for the API origin must "Bypass Access for HTTP OPTIONS" regardless of path. See `apps/web/README.md` for the full decision table.
