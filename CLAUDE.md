...
**One canonical HTTP client per external vendor** — every SEC EDGAR call goes through `alphalens/data/alt_data/sec_edgar_client.py::SecEdgarClient`; every Alpha Vantage call through `alphavantage_client.py::AlphaVantageClient`; every Gemini call through `gemini_client.py::GeminiClient`. Why: rate caps are per-IP / per-key — shadow clients fragment the request stream and break quota tracking. Sites with an injected client (watchdog `SECEdgarSource`, `CIKLoader`; thematic mapper/extractor/generator) keep DI; module-level helpers call the respective `get_default_*_client()`. Env vars: `SEC_EDGAR_USER_AGENT`, `ALPHA_VANTAGE_API_KEY`, `GOOGLE_API_KEY`. Enforcement: `tests/test_no_raw_sec_http.py`, `tests/test_no_raw_av_http.py`, `tests/test_no_raw_gemini_sdk.py` — each with a positive-control case so the regex / URL-list cannot rot to empty silently. Design memos: `docs/research/{sec_edgar,alphavantage,gemini}_client_consolidation_2026_05_*.md`.
