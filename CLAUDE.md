...
**One canonical HTTP client per external vendor (SEC EDGAR pinned 2026-05-19, Alpha Vantage pinned 2026-05-20)** — every SEC EDGAR HTTP call in the repo MUST go through `alphalens/data/alt_data/sec_edgar_client.py::SecEdgarClient`; every Alpha Vantage HTTP call MUST go through `alphalens/data/alt_data/alphavantage_client.py::AlphaVantageClient`. Sites with an injected client (watchdog `SECEdgarSource`, `CIKLoader`) keep DI; module-level functions (thematic `tenk_grep` / `etf_holdings`, fundamentals fetcher, av_earnings default fetcher) call the respective `get_default_*_client()`. Env vars: `SEC_EDGAR_USER_AGENT` (one var, fallback `ALPHALENS_DEFAULT_USER_AGENT`) for SEC, `ALPHA_VANTAGE_API_KEY` for AV. Enforcement: `tests/test_no_raw_sec_http.py` + `tests/test_no_raw_av_http.py` fail red on `urlopen(` / `requests.get(` / `urllib.request.*` / `httpx.*` / `aiohttp.*` in any file that also mentions a vendor URL fragment. Rationale: SEC's 10 req/s fair-access cap is per-IP and AV's free-tier 25/day quota is per-API-key — any uncoordinated shadow client risks taking down EVERY consumer at once (paradigm-14 PEAD backfill, Layer 1 watchdog, thematic verification, EDGAR fundamentals; for AV: fundamentals gate, EARNINGS bulk cache). Same policy will extend to Gemini in a follow-up PR per the 2026-05-19 vendor audit. Design memos: `docs/research/sec_edgar_client_consolidation_2026_05_19.md`, `docs/research/alphavantage_client_consolidation_2026_05_20.md`.
