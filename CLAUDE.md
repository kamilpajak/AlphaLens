...
**One canonical HTTP client per external vendor (SEC EDGAR pinned 2026-05-19, Alpha Vantage + Gemini pinned 2026-05-20)** — every SEC EDGAR HTTP call MUST go through `alphalens/data/alt_data/sec_edgar_client.py::SecEdgarClient`; every Alpha Vantage HTTP call MUST go through `alphalens/data/alt_data/alphavantage_client.py::AlphaVantageClient`; every Gemini call MUST go through `alphalens/data/alt_data/gemini_client.py::GeminiClient`. Sites with an injected client (watchdog `SECEdgarSource`, `CIKLoader`; thematic mapper/extractor/generator hoisting) keep DI; module-level helpers (thematic `tenk_grep` / `etf_holdings`, fundamentals fetcher, av_earnings default fetcher, backtest llm_scorers, analyze_rejections script) call the respective `get_default_*_client()`. Env vars: `SEC_EDGAR_USER_AGENT` (fallback `ALPHALENS_DEFAULT_USER_AGENT`) for SEC, `ALPHA_VANTAGE_API_KEY` for AV, `GOOGLE_API_KEY` for Gemini. Enforcement: `tests/test_no_raw_sec_http.py` + `tests/test_no_raw_av_http.py` + `tests/test_no_raw_gemini_sdk.py` fail red on raw HTTP / SDK-import / shadow-client construction in any file that also mentions the vendor URL fragment (SEC, AV) or imports the vendor SDK (Gemini). Each enforcement test ships a positive-control case so a regex / URL-list that rots to empty cannot let a shadow client slip through silently. Rationale: SEC's 10 req/s fair-access cap is per-IP, AV's free-tier 25/day quota is per-API-key, and Gemini's per-key quota / billing accounting all break when multiple shadow clients fragment the request stream — one canonical client also gives one place to inject quota tracking or retries later. Design memos: `docs/research/sec_edgar_client_consolidation_2026_05_19.md`, `docs/research/alphavantage_client_consolidation_2026_05_20.md`, `docs/research/gemini_client_consolidation_2026_05_20.md`.
