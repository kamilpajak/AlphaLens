...
- **Origin** — Cloudflare Tunnel from VPS to `api.<domain>` mapped to `localhost:8000` (Django gunicorn behind Tunnel). CF Access (Service Tokens) authenticates the SPA → API path; `auth_cf` middleware validates `CF-Access-Jwt-Assertion` headers.
