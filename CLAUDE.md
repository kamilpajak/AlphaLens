...
**One canonical HTTP client per external vendor** — every SEC EDGAR call goes through `alphalens_pipeline/data/alt_data/sec_edgar_client.py::SecEdgarClient`; every Alpha Vantage call through `alphavantage_client.py::AlphaVantageClient`; every OpenRouter (DeepSeek v4 Pro/Flash) call through `openrouter_client.py::OpenRouterClient`; every Polygon call through `polygon_client.py::PolygonClient`. `gemini_client.py::GeminiClient` remains for the research-side `alphalens_research/backtest/llm_scorers.py` (out-of-thematic-pipeline) until that swap lands as a follow-up PR. Why: shadow clients fragment the request stream and break quota tracking — and the quota scope differs per vendor:
