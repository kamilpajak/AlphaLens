...
| `alphalens-paper-submit.{service,timer}` | Mon-Fri oneshot at 13:25 UTC (`OnCalendar=Mon..Fri *-*-* 13:25:00 UTC`, NO `Persistent=true`) with `ExecCondition=alphalens paper is-trading-day` gating on US holidays | `/bin/sh -c '.venv/bin/alphalens paper submit --date $(date -u +%Y-%m-%d)'` | Alpaca paper account orders + `~/.alphalens/paper_ledger.db` | ~5-30s | LIVE pending — PR-D (epic #295 #298). Submits entry-tier limits 5 min pre-XNYS opening cross. |
| `alphalens-paper-reconcile.{service,timer}` | Mon-Fri every 30 min 14:00-21:00 UTC (`OnCalendar=Mon..Fri *-*-* 14..21:00,30:00 UTC`, NO `Persistent=true`) with same `ExecCondition` holiday gate | `.venv/bin/alphalens paper reconcile` (NO `--date`, sweeps all OPEN orders across dates) | `~/.alphalens/paper_ledger.db` order-status updates | ~5-15s/fire | LIVE pending — PR-D (epic #295 #298) |
