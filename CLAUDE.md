...
**One canonical HTTP client per external vendor** — every SEC EDGAR call goes through `alphalens_pipeline/data/alt_data/sec_edgar_client.py::SecEdgarClient`; every Alpha Vantage call through `alphavantage_client.py::AlphaVantageClient`; every OpenRouter (DeepSeek v4 Pro/Flash) call through `openrouter_client.py::OpenRouterClient`; every Polygon call through `polygon_client.py::PolygonClient`. `gemini_client.py::GeminiClient` now remains ONLY for the ad-hoc `apps/alphalens-research/scripts/analyze_rejections.py` rejection-analysis script; the backtest `llm_scorers` and the thematic pipeline both route through OpenRouter/DeepSeek. Why: shadow clients fragment the request stream and break quota tracking — and the quota scope differs per vendor:
...
| Gemini (Google AI) — *research-side only* | **per API key + per project** | Variable per model | Used by the ad-hoc `scripts/analyze_rejections.py`; backtest llm_scorers + thematic pipeline migrated to OpenRouter. |

Sites with an injected client (edgar_detector `SECEdgarSource`, `CIKLoader`; thematic mapper/extractor/generator; thematic press verification + news ingest; `PolygonShortInterestClient` domain wrapper) keep DI; module-level helpers call the respective `get_default_*_client()`. Env vars: `SEC_EDGAR_USER_AGENT`, `ALPHA_VANTAGE_API_KEY`, `OPENROUTER_API_KEY`, `POLYGON_API_KEY` (plus legacy `GOOGLE_API_KEY` for the ad-hoc `analyze_rejections.py` script). Enforcement: `apps/alphalens-research/tests/test_no_raw_sec_http.py`, `apps/alphalens-research/tests/test_no_raw_av_http.py`, `apps/alphalens-research/tests/test_no_raw_openrouter_http.py`, `apps/alphalens-research/tests/test_no_raw_gemini_sdk.py`, `apps/alphalens-research/tests/test_no_raw_polygon_http.py` — each with a positive-control case so the regex / URL-list cannot rot to empty silently. Design memos: `docs/research/{sec_edgar,alphavantage,gemini,polygon}_client_consolidation_2026_05_*.md`.
...
| `alphalens-thematic-build.{service,timer}` | 6× daily oneshot at HH:30 UTC (00/04/08/12/16/20) with `RandomizedDelaySec=5min` + `Persistent=true` + `TimeoutStartSec=45min`, wrapping `docker run --rm alphalens-pipeline` + `compose run --rm rebuild-cache` (Django stack) | `alphalens thematic {ingest,extract,map-themes,score,brief}` (ingest passes `--force`) + `manage.py rebuild_briefs_cache` | `~/.alphalens/thematic_briefs/` + Postgres `briefs`/`days_meta` tables | ~12-20 min | LIVE 2026-05-30 — 6×/day cadence per PR-F (epic #295 #300) maps onto global exchange rotation (XTKS/XHKG/XSHG/XWAR/XNYS); feeds CF Pages SPA via Django API |
...
| `alphalens-paper-plan.{service,timer}` | Mon-Fri oneshot at 13:05 UTC (`OnCalendar=Mon..Fri *-*-* 13:05:00 UTC`, NO `Persistent=true`) with `ExecCondition=alphalens paper is-trading-day` (the ONLY holiday gate — plan has no internal market-closed guard) | `/bin/sh -c '.venv/bin/alphalens paper plan --use-test-account --date $(date -u -d yesterday +%Y-%m-%d)'` | PLANNED rows in `~/.alphalens/paper_ledger.db` | ~5-15s | LIVE pending — PR-D follow-up (epic #295 #298, PR #343). First link in the chain; reads that morning's `(D-1)` brief parquet, writes PLANNED ladder rows for submit. |
| `alphalens-paper-submit.{service,timer}` | Mon-Fri oneshot at 13:25 UTC (`OnCalendar=Mon..Fri *-*-* 13:25:00 UTC`, NO `Persistent=true`) with `ExecCondition=alphalens paper is-trading-day` gating on US holidays | `/bin/sh -c '.venv/bin/alphalens paper submit --use-test-account --date $(date -u -d yesterday +%Y-%m-%d)'` | Alpaca **test** paper account orders + `~/.alphalens/paper_ledger.db` | ~5-30s | LIVE pending — PR-D (epic #295 #298). Submits the PLANNED rows plan wrote (same `(D-1)` brief) as entry-tier limits 5 min pre-XNYS opening cross. |
| `alphalens-paper-reconcile.{service,timer}` | Mon-Fri every 30 min 14:00-21:00 UTC (`OnCalendar=Mon..Fri *-*-* 14..21:00,30:00 UTC`, NO `Persistent=true`) with same `ExecCondition` holiday gate | `.venv/bin/alphalens paper reconcile --use-test-account` (NO `--date`, sweeps all OPEN orders across dates) | `~/.alphalens/paper_ledger.db` order-status updates | ~5-15s/fire | LIVE pending — PR-D (epic #295 #298) |
| `alphalens-feedback-shadow-returns.{service,timer}` | daily oneshot at 06:30 UTC (`OnCalendar=*-*-* 06:30:00 UTC` + `Persistent=true` + `TimeoutStartSec=45min`) | `/bin/sh -c` → `.venv/bin/alphalens feedback backfill-shadow-returns --account test` (sweeps last 14 brief dates newest→oldest, prices each matured `(D)` horizon via Polygon minute bars; NO `--date`) | `~/.alphalens/feedback.db` `shadow_return`/`realized_return` columns | ~5-30min (Polygon ~5 req/min throttle × matured tickers in the window) | LIVE 2026-06-02 — Track A v2 PR-T (PR #358, epic #301). `POLYGON_API_KEY` present in `/etc/alphalens/env`; `POLYGON_LIVE_TEST` probe passed; first manual run emitted the job-success metric. `AlphalensJobStale`@48h (sweep exits 0 nightly → staleness catches "stopped running"). |
