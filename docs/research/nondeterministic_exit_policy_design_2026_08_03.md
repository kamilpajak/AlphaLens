...
- **No policy resolution in the protection pass (adversarial-review CRITICAL, 2026-08-03).** `resolve_exit_policy` raises `ValueError` on an unknown name, and `_maybe_reanchor` runs inside `reconcile_protection` — a single unwrapped pure call (`control_loop.py:412`) whose raise would starve EVERY position that tick (INV-3/INV-4). The policy MUST be resolved once at startup (`build_default_deps`, fail-fast) and cached on `LoopDeps` + `ProtectionView`; the hot paths read the cached instance. Enforced by a source-guard test. See the plan's Task 3 + the "Adversarial review outcome" section in `docs/research/nondeterministic_exit_policy_plan_2026_08_03.md`.
