...
Standing rule adopted with the frame choice: demand **≥5% headroom** between candidate gross and the gross budget — the budget is `GROSS_FRAC` × mark-to-market `total_value` (0.5 at lock time, 1.0 since 2026-09-08 — LIVE daemon memo §8 point 3 amendment), which drifts daily (EOD netting, FX, open-position marks); a sub-2% margin makes pass/refuse a coin flip decided by the fx sizing buffer and per-tier qty flooring, and a refuse at drain time is terminal (§4.3).
...
| 2 | `GROSS_FRAC` pin | 0.25 → 0.5 (boot-assert max; the repaired cap makes 0.5 mean a REAL 50%). *Amended 2026-09-08: 0.5 → 1.0 with ten slots — risk bounded by diversification across names, 1.0 = the no-leverage line.* Mode-independent: `safety.check` runs before sizing for every armed pick (`control_loop.py:3089`) with no knowledge of the sizing mode — binds identically in `clamped` and `declared`, and survives into Path B as gate 3 of the §5 stack, always denominated against REAL equity, never the frame |
