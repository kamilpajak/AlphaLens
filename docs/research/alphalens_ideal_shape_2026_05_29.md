...
- ⏳ **v2 PR-3 — `shadow_return` + `realized_pnl` computation** — NULL-capable columns shipped in PR-1; the §4 arrival-VWAP counterfactual is deferred until a minute-bar source is wired (needs `PolygonClient.get_agg_range`)
...
4. **Feedback ledger v2 — outcome join** (Track A v2) — links each decision to its paper-trade outcome [✅ PR-1 schema + join plumbing SHIPPED PR #351; `shadow_return`/PnL computation deferred to PR-3]
