...
4. **Feedback ledger v2 — outcome join** (Track A v2) — links each decision to its paper-trade outcome [✅ PR-1 schema + join plumbing #351, ✅ PR-2 VIX cache #354, ✅ PR-3 `shadow_return` + `realized_return` #356, ✅ PR-T nightly timer #358, ✅ PR-4 execution-mode gating (inert) #359, ✅ VIX-cache staleness alert #376 — all SHIPPED; remaining: PR-5 wire the gate to the live submitter at ≥50 decisions + the VPS deploy batch (timer units + keys + image rebuild + hand-sync VIX rules)]
