...
- **Fill is an execution overlay** on top: `FILLED` adds a `realized_return`; `UNFILLED` carries only the `shadow_return`. The scorer re-weights on `shadow_return` (signal quality); the gap `realized_return − shadow_return` measures **execution efficiency** (the §6 cost drag) — so execution quality never leaks into signal evaluation. (Shipped PR-3 anchors `shadow_return` to the next-session opening-window VWAP rather than the true surfacing instant — the ledger has no per-candidate intraday timestamp — which is the strongest honest, transactable anchor available; both `shadow_return` and `realized_return` are decimal fractions so the gap is unit-consistent.)
...
- ✅ **v2 PR-1 — outcome join schema + plumbing (PR #351, SHIPPED 2026-06-01)** — 6 nullable outcome columns + `PRAGMA user_version` migration; `outcome_join.join_decision_outcomes()` links a decision to its paper `plan_outcomes` row by `(brief_date, ticker, account)` and stamps `fill_status` (FILLED / **UNFILLED** / PARTIAL — non-fills captured, §4) / `exit_kind` / `outcome_plan_id`; `alphalens feedback join-outcomes` CLI; read-only `outcome_*` on the Django decision response
- ✅ **v2 PR-3 — `shadow_return` + `realized_return` computation (PR #356, SHIPPED 2026-06-01)** — the §4 arrival-price counterfactual is now real: `shadow_return = (horizon_vwap − arrival_vwap) / arrival_vwap`, arrival = VWAP of the first 30 min of the first XNYS session on-or-after `brief_date`, horizon = same window +5 trading sessions; computed for **every** matured outcome incl. UNFILLED (no look-ahead — never reads the realised exit). `realized_return` (`(blended_exit − blended_entry)/blended_entry`, FILLED only) **replaces** PR-1's `realized_pnl` (schema gen 1→2 rename) so both are decimal fractions for the unit-consistent §6 gap. New `PolygonClient.get_agg_range` minute bars + `paper.calendar` session helpers; `alphalens feedback compute-shadow-returns` CLI (separate rate-limited pass from join-outcomes). **Matured-horizon fail-loud guard:** skips the run with a loud WARNING if the +5-session horizon is not strictly in the past (Polygon serves only closed sessions) rather than silently stamping nothing
- ✅ **v2 PR-2 — VIX server-side cache (PR #354, SHIPPED 2026-06-01)** — `market_regime_at_entry` now stamps a real low/mid/high bucket instead of always-"unknown". `regime.get_cached_vix()` reads a tiny JSON cache on the POST hot path (one local file read, zero network; 96h staleness on `fetched_at`, degrades to "unknown" on any miss/stale/error); `alphalens cache refresh-vix` force-pulls VIXCLS via the canonical FREDClient (throwaway cache_dir) on the 6×/day thematic build. Observability follow-up SHIPPED in #376 (below)
- ✅ **v2 PR-T — nightly shadow-return VPS timer (PR #358, SHIPPED 2026-06-01)** — `alphalens feedback backfill-shadow-returns` sweeps the last 14 brief dates (newest→oldest so a rate-limit timeout never starves the just-matured dates) and prices each whose +5-session horizon matured; idempotent + `Persistent=true` catch-up. `alphalens-feedback-shadow-returns.{service,timer}` fires daily 06:30 UTC. Staleness rule ADDED (not exempt) at 48h — the sweep exits 0 nightly, so `AlphalensJobStale` catches "stopped running" which the exit-code alert cannot. VPS deploy (install units + `POLYGON_API_KEY` + rebuild image + `POLYGON_LIVE_TEST` probe) is the remaining operator step
- ✅ **v2 PR-4 — execution-mode gating, INERT-by-default (PR #359, SHIPPED 2026-06-01)** — the §6 limit→market Perold break-even per regime, read-only. `recommend_execution_modes()` switches limit→market iff shrunk `(1−fill_rate)·MO − MI − fill_rate·max(g,0) > 0`, with empirical-Bayes shrinkage toward the pool. Two hard gates: pooled n<50 → all LIMIT (inert; fires for every cell today); cell n<30 → adopt pooled. Safe-default LIMIT on every undefined/non-finite/negative-MO/unknown path. `alphalens feedback execution-modes` report (GATE INERT banner + per-stat backing-n). Gates on REGIME ONLY (theme/template deferred — `template_id` not on the decision row). Maturity = `shadow_return IS NOT NULL` (not `outcome_computed_at`) + dedup by `(brief_date, ticker)` (both adversarial-critique CRITICALs). **Does NOT wire the live submitter** — that is execution (capital_deploy_clause); deferred to PR-5 once a ≥50 validated sample exists
- ✅ **v2 VIX-cache staleness alert (PR #376, SHIPPED 2026-06-02)** — closes the PR-2 observability deferral. `alphalens cache refresh-vix` emits `alphalens_vix_cache_fetched_at_timestamp_seconds{series="VIXCLS"}` (gauge, after the durable write, non-fatal `except Exception`); `AlphalensVixCacheStale` (72h = 0.75× the 96h reader ceiling → fires ~24h **before** the reader starts degrading stamps to "unknown") + `AlphalensVixCacheMetricMissing`. Distinct alertname + no `job=` label keeps it out of the per-job cron enumerations (the refresh runs inline in `run_thematic_day.sh`, not a systemd unit). A dead FRED refresher was the silent single point of failure for the entire regime axis the ledger conditions on. **DEPLOYED to the VPS 2026-06-02** (rules synced to live Prometheus + HUP, 0 false-fires) — but only after **PR #377** (`fix: route container metrics to the node_exporter scrape dir`): the thematic-build container was emitting the VIX + #373/#374 stage gauges into `~/.alphalens/metrics`, which the live node_exporter does NOT scrape (it reads `/var/lib/node_exporter/textfile`), so they were silently orphaned. #377 adds an explicit `-e ALPHALENS_TEXTFILE_DIR` + identity mount and de-stales the repo docs that contradicted live. Verified: `curl :9100/metrics` now returns all 5 stage gauges + the VIX gauge
- ⏳ **v2 PR-5 — wire the gate to the live paper submitter** — gated on a production-validated ≥50-decision pooled sample; the `submit_market_order` path + injection point already exist
...
4. **Feedback ledger v2 — outcome join** (Track A v2) — links each decision to its paper-trade outcome [✅ PR-1 schema + join plumbing #351, ✅ PR-2 VIX cache #354, ✅ PR-3 `shadow_return` + `realized_return` #356, ✅ PR-T nightly timer #358, ✅ PR-4 execution-mode gating (inert) #359, ✅ VIX-cache staleness alert #376, ✅ container-metric scrape-routing fix #377 — all SHIPPED + the VPS deploy batch (timer + image rebuild + rules sync) DEPLOYED 2026-06-02; remaining: PR-5 wire the gate to the live submitter at ≥50 decisions]
...
| Date | Change | Reason |
|------|--------|--------|
| 2026-06-02 | §9 Track A + §10 — Track A v2 VPS deploy batch DEPLOYED + container-scrape fix PR #377 | Brought the whole Track A v2 backend live on the VPS: host venv `git pull`+`uv sync`, PR-T shadow-returns timer installed + enabled (06:30 UTC, `POLYGON_LIVE_TEST` passed, first run emitted its job metric), `alphalens-pipeline:latest` rebuilt, Prometheus rules synced to the live file + `kill -HUP 1` (0 false-fires). **Surfaced + fixed a pre-existing prod-scrape bug (PR #377):** the thematic-build container emitted the #373/#374 stage gauges + the #376 VIX gauge into `~/.alphalens/metrics`, but the live node_exporter scrapes `/var/lib/node_exporter/textfile` — so all container gauges were silently orphaned (the #373 dead-man-switch alerts never had input series; the VIX `MetricMissing` would have false-fired on `absent()` forever). Fix: explicit `-e ALPHALENS_TEXTFILE_DIR=/var/lib/node_exporter/textfile` + identity `-v` mount on the thematic-build docker run, + de-staled the 3 repo docs that wrongly claimed `~/.alphalens/metrics` was the scraped dir. Found via the live-probe doctrine (CI green ≠ prod scrape); verified live with `curl :9100/metrics`. Workflow Map→Design→Critique gated the fix — the first critique REJECTED on an unreconciled repo-vs-live contradiction until `docker inspect node-exporter` proved the live scrape dir. |
| 2026-06-02 | §9 Track A + §10 — v2 VIX-cache staleness alert SHIPPED PR #376 (closes the PR-2 observability deferral) | A dead FRED refresher silently ages the VIX cache past the 96h reader ceiling → every new decision stamps `market_regime_at_entry='unknown'` with NO signal, poisoning the per-regime PR-4/PR-5 execution-mode cells at the source. `alphalens cache refresh-vix` now emits `alphalens_vix_cache_fetched_at_timestamp_seconds{series="VIXCLS"}` (gauge = epoch of the durable write, emitted inside `refresh_vix_cache`, broad `except Exception` non-fatal like the other emit callsites). Paired rules: `AlphalensVixCacheStale` (`time() - metric > 259200` = **72h = 0.75× the 96h ceiling**, fires ~24h BEFORE degradation = a day of runway; 18 missed 4-hourly runs = unambiguously dead, no false page) + `AlphalensVixCacheMetricMissing` (`absent(...)`, no humanizeDuration). DISTINCT alertname + NO `job=` label so it stays out of the per-job cron enumerations (refresh runs inline in `run_thematic_day.sh`, not a systemd unit) — own regression pins in `TestVixCacheStaleness`. Workflow Map→Design→Critique picked it as the highest-value NON-data-blocked feedback work (everything else inert until first matured shadow ~2026-06-08). Zen pre-merge (DeepSeek v4 Pro): 1 MEDIUM (threshold 120h→72h, fire before degradation) + 2 LOW (broaden `except OSError`→`Exception`; scope rules to `{series="VIXCLS"}`) applied as follow-up. Full research suite 3705 + promtool 20 rules. **Remaining VPS step:** hand-sync repo rules → live Prometheus file + confirm node_exporter actually scrapes the metric (CI green ≠ prod scrape, live-probe doctrine). |
| 2026-06-01 | §9 Track A + §10 — v2 PR-T (nightly timer) + PR-4 (execution-mode gating) SHIPPED PR #358 + #359 | **PR-T (#358):** `alphalens feedback backfill-shadow-returns` + `alphalens-feedback-shadow-returns.{service,timer}` (daily 06:30 UTC, Persistent). The sweep loops the unchanged single-date `compute_shadow_returns` over `[today-14d, today]` newest→oldest (a rate-limit `TimeoutStartSec` kill never starves the just-matured dates; older priced dates re-covered next night via idempotency). Staleness rule ADDED not exempted (the design memo assumed weekly — it's nightly, exits 0 every night, so `AlphalensJobStale`@48h catches "stopped running" which the exit-code alert can't); auto-covered by the glob-derived metrics-hook + prometheus-parity tests. **PR-4 (#359):** `execution_modes.recommend_execution_modes` — Perold limit→market break-even per regime with empirical-Bayes shrinkage toward the pool; two hard gates (pooled n<50 → all LIMIT inert; cell n<30 → adopt pooled); safe-default LIMIT on every undefined path; `market` returned in one guarded branch only. READ-ONLY (`alphalens feedback execution-modes` report) — does NOT wire the live submitter (execution = capital_deploy_clause; PR-5 at ≥50). Two adversarial-critique CRITICALs folded in pre-code: maturity = `shadow_return IS NOT NULL` (the cheap fill-status join sets `outcome_computed_at` without pricing → would inflate the gate) + dedup by `(brief_date, ticker)` (the ticker-day join stamps identical outcomes onto every multi-theme row). Regime-only gating (template_id not on the decision row). Both zen pre-merge (DeepSeek v4 Pro): PR-T 0 crit/high, PR-4 0 crit/high/medium; minor LOW follow-ups applied. VPS deploy of the timer (install units + `POLYGON_API_KEY` + rebuild image + live probe) is the remaining operator step. |
| 2026-06-01 | §4 + §9 Track A + §10 — v2 PR-3 (`shadow_return` + `realized_return`) SHIPPED PR #356 | The §4 arrival-price counterfactual is now computed: `shadow_return = (horizon_vwap − arrival_vwap)/arrival_vwap`, arrival = VWAP of the first 30 min of the first XNYS session on-or-after `brief_date`, horizon = same window +5 trading sessions; for every matured outcome incl. UNFILLED, no look-ahead. `realized_pnl` renamed to `realized_return` (`(blended_exit − blended_entry)/blended_entry`, FILLED only; schema gen 1→2) so both are decimal fractions for the unit-consistent §6 gap. New `PolygonClient.get_agg_range` + `paper.calendar` session helpers (`session_on_or_after` / `advance_trading_sessions` / `session_open_utc`); `stamp_outcome` sentinel so the cheap fill-status join never wipes a computed shadow value; `alphalens feedback compute-shadow-returns` CLI. **Matured-horizon fail-loud guard** (Polygon serves only closed sessions → skip+WARN, never silent-empty) + per-ticker resilience + implausible-move (>60%, split) guard + opt-in `POLYGON_LIVE_TEST` probe. Anchor is next-session opening VWAP (no per-candidate surfacing instant in the ledger) — strongest honest transactable anchor. Built via Map→Design workflow + adversarial critique; zen pre-merge (DeepSeek v4 Pro): SOUND, private-import coupling fixed (made `EXIT_KIND_TO_FILL_STATUS` public), one-day maturity-latency LOW accepted as a deliberate nightly-job choice; CI bandit B608 on the dynamic SET clause suppressed with `# nosec`. Deferred: nightly VPS timer + PR-4 execution-mode gating (≥50 decisions). |
| 2026-06-01 | §9 Track A — v2 PR-2 (VIX server-side cache) SHIPPED PR #354 | `market_regime_at_entry` stamps a real low/mid/high bucket instead of always-"unknown". `regime.get_cached_vix()` reads a tiny JSON cache on the POST hot path (one local file read, zero network, stdlib-only); 96h staleness on `fetched_at` (sole freshness gate), degrades to "unknown" on any miss/stale/error so a dead refresher never blocks a decision. `alphalens cache refresh-vix` force-pulls VIXCLS via the canonical FREDClient into a throwaway cache_dir (FREDClient has no TTL → never re-reads/races the shared parquet), atomic JSON write; wired best-effort into `run_thematic_day.sh` (6×/day) + `-e FRED_API_KEY`. Map→Design workflow picked FRED over yfinance (already canonical via `signals.vix_decile`). Zen pre-merge (DeepSeek v4 Pro): CRITICAL observability → deferred Prometheus staleness alert (rules hand-synced on VPS, out of repo) + loud stderr warn; 2 MEDIUM applied (now-before-read, stale round-trip test); HIGH NaN-guard rejected (dropna + empty-check already guarantee non-NaN). |
| 2026-06-01 | §9 Track A + §10 — v2 PR-1 (outcome-join schema + plumbing) SHIPPED PR #351 | First edge of the §4 outcome-join is now real: 6 nullable outcome columns (`outcome_plan_id`, `fill_status`, `exit_kind`, `shadow_return`, `realized_pnl`, `outcome_computed_at`) on the `decisions` table via a `PRAGMA user_version` ALTER migration (`table_info`-guarded, idempotent fresh+legacy); `outcome_join.join_decision_outcomes()` links a decision to its paper `plan_outcomes` row by `(brief_date, ticker, account)` — captures **UNFILLED** candidates, not just survivors (§4 anti-survivorship); `alphalens feedback join-outcomes` CLI; read-only `outcome_*` on the Django response. `shadow_return`/`realized_pnl` ship NULL-capable — the §4 arrival-VWAP computation is deferred to PR-3 (no minute-bar source exists yet). Built via a Map→Design workflow (6 parallel readers + adversarial critique) + zen pre-merge (DeepSeek v4 Pro: KeyError-guard on unmapped exit_kind + case-insensitive ticker match applied as follow-up). Scope refinement recorded on issue #165. |
