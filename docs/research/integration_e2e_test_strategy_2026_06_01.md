...
**Status:** IN PROGRESS — Phases 1a (#361/#362), 1b (#355), 2 (#364), 3 (#366, L3 brief-stage golden replay + VCR cassette infra) SHIPPED. **Phase 3b COMPLETE** (all 5 thematic stages now have an L3 golden), one stage per PR: 3b-1 extract (#368), 3b-2 map-themes (#369, generic `VendorCassette`), 3b-3 ingest (#370, partial — GDELT synthetic + Polygon/RSS live; EDGAR deferred), **3b-4 score (#371 — frozen at the feature/insider fetch boundary; the ~764-ticker companyfacts universe is too big to cassette)**. **Phase 2b SHIPPED** (#372 — startup migration-skew guard + a Postgres reproduction of #331/#340). **Phase 4 SHIPPED** (this PR — per-stage thematic output-volume gauges + zero-output-with-nonempty-input + brief-volume-anomaly alerts). Phase 5 still open.
**Date:** 2026-06-01
