...
**Status:** COMPLETE — Phases 1a (#361/#362), 1b (#355), 2 (#364), 3 (#366, L3 brief-stage golden replay + VCR cassette infra) SHIPPED. **Phase 3b COMPLETE** (all 5 thematic stages now have an L3 golden), one stage per PR: 3b-1 extract (#368), 3b-2 map-themes (#369, generic `VendorCassette`), 3b-3 ingest (#370, partial — GDELT synthetic + Polygon/RSS live; EDGAR deferred), **3b-4 score (#371 — frozen at the feature/insider fetch boundary; the ~764-ticker companyfacts universe is too big to cassette)**. **Phase 2b SHIPPED** (#372 — startup migration-skew guard + a Postgres reproduction of #331/#340). **Phase 4 SHIPPED** (#373 — per-stage thematic output-volume gauges + zero-output-with-nonempty-input + brief-volume-anomaly alerts). **Phase 5 SHIPPED** (this PR — 4 env-gated L4 live vendor probes + a shared transient/permanent classifier + a weekly schedule-only CI `live-probes` job + `just probe-live`/`test-deploy-shape`; the VPS post-deploy gate `postdeploy_check.sh` was already wired into the runbook in 1a/#362). **Epic complete.**
**Date:** 2026-06-01
...
| **2b — migration-skew guard + Postgres reproduction** (fills a gap no layer covered, §10) — ✅ SHIPPED (this PR) | **Rejected the literal two-container compose test**: the incident is not "two containers" but "one shared Postgres at migration 0006 while the running code expects 0007", which reproduces faithfully against a single Postgres via Django's own `MigrationExecutor` — no testcontainers (absent from `uv.lock`), no two-image orchestration, identical signal at far lower CI cost/flakiness. **Durable fix = a startup schema-skew guard, NOT an advisory-lock** (the lock fixes a concurrent-migrate race; this is a version-skew where a stale image's code simply disagrees with the already-migrated schema). Shipped: `briefs/migration_guard.py::assert_schema_current` checks BOTH directions — code-ahead-of-DB (`migration_plan` non-empty) AND **DB-ahead-of-code** (migrations recorded in `django_migrations` that the code's graph doesn't know — the literal #331/#340 direction a plain "unapplied" check would miss), wired as the first line of `rebuild_briefs_cache.handle` so the one-shot refuses to run (clean `CommandError`, non-zero exit) instead of dying per-row. Tests: `test_migration_guard.py` (hermetic/SQLite — pure `detect_skew` classifier both directions + no-op-on-synced-DB + command-wiring) and `test_migrate_skew.py` (Postgres-only, `transaction=True` + `MigrationExecutor` rolls the DB back to 0006 while the model stays at head → asserts `ProgrammingError`/sqlstate 42703/`llm_confidence` **and** `Brief.objects.count()==0`, plus a head-state control). CI: a `postgres:16-alpine` `services:` block on the `django` job + a dedicated skew step setting `DATABASE_URL` at it; the main coverage step stays SQLite, the skew test self-skips on any non-Postgres engine (so local `pytest` needs no DB). Advisory-lock concurrency hardening left as a separate, lower-priority item (not bundled here). |
| **3 — L3 golden-master replay** — ✅ SHIPPED #366 (brief stage) | `scripts/record_golden_brief.py` one-time live capture; VCR-style `ReplayOpenRouter`/`RecordingOpenRouter` cassettes (keyed on full request descriptor, fail-loud) + frozen scored/OHLCV/golden fixtures; golden = schema+row-count+tickers+stable exemplar; `test_golden_brief_replay` (research) + `test_golden_api` (Django) with side-effect invariants (#3 non-empty, #6 ingest-drop); `just test-golden`. **Phase 3b** (one stage/PR): 3b-1 extract (#368) reuses the LLM cassette + 3 synthetic template rows; 3b-2 map-themes (this PR) adds the generic non-LLM `VendorCassette` (Polygon firehose, trimmed to candidate rows) + frozen tenk 10-K text / Form-4 / catalyst / yfinance-mcap seams — all monkeypatched at existing orchestrator seams, zero prod change; 3b-3 ingest (this PR) adds `url_cassette.py` (GDELT `UrlJsonCassette` + RSS `FeedCassette`) and locks the cross-source merge over GDELT (synthetic — free-tier 429s a live sweep) + Polygon/RSS (live), EDGAR deferred (nested-fetch volume); 3b-4 score (#371) freezes at the scorer's own `_build_feature_fetcher` / `score_insider` / mcap fetch boundary (the ~764-ticker SIC peer-cohort companyfacts universe is ~57MB — too big to cassette; both are data-layer deps) and drives the real percentile-rank / magic-formula / technicals / catalyst / `compose_weighted_score` logic over frozen feature+insider+OHLCV+catalyst fixtures. **Phase 3b complete.** |
| **4 — runtime output-volume metrics** (silent-noop, leverages existing observability epic, §10) — ✅ SHIPPED (this PR) | Each of the 5 thematic stages emits an input/output row-count gauge pair `alphalens_thematic_stage_{input,output}_rows{stage="..."}` via the existing `emit_domain_metrics` helper (per-stage `thematic-<stage>` job so the 5 sequential processes don't clobber one `.prom` file; brief folds into its existing `thematic-build` emit). Escapes Docker for free (thematic-build unit's `HOME=/app/home` + `~/.alphalens` bind-mount lands the textfile on the host node_exporter dir). Two rules in the existing `alphalens-cron-health` group: `AlphalensThematicStageZeroOutput` (`max_over_time(output[24h])==0 and max_over_time(input[24h])>0` — gauge-correct, `__name__`-stripped so input/output match on `{stage}`) + `AlphalensThematicBriefVolumeAnomaly` (brief < 34% of 7-day median, `>=3` cold-start guard). Thresholds first-guess, tunable. Parity tests unaffected (not `AlphalensJobStale`, no new unit). Activation is a separate operator step (rebuild pipeline image + hand-sync the live Prometheus rules + HUP — the live file is not the repo file). |
| **5 — L4 live probes + VPS gate** — ✅ SHIPPED (this PR) | New `apps/alphalens-research/tests/live/` package: a shared classifier (`run_probes` + `PermanentProbeError`/`TransientProbeError`) generalising the GDELT transient(429/timeout)/permanent(shape/404/empty) + >50%-majority logic, pinned by a non-gated hermetic `test_live_probe_helper.py` (the one module in the dir that runs in default CI). Four env-gated probes, shape-only (assert non-emptiness + keys/finish_reason/schema, NEVER values): **SEC** (`SEC_LIVE_TEST`, pins #2 — DISCOVERS a real EX-99.1 from Apple's live recent-8-K feed rather than pinning a fabricated/aging accession, the open-q6 answer); **OpenRouter** (`OPENROUTER_LIVE_TEST`, pins #3 — asserts 200 + `finish_reason != MAX_TOKENS` + `EVENT_RESPONSE_SCHEMA` conformance for both DeepSeek model ids; a 404 is permanent = the retirement signal); **Polygon** (`POLYGON_LIVE_TEST`, moved from `tests/test_polygon_live.py`); **yfinance** (`YFINANCE_LIVE_TEST` — OHLCV + live mcap + next-earnings seams). `just probe-live` (all flags) + `just test-deploy-shape` (prints the VPS gate recipe). A schedule-only `live-probes` CI job (`if: github.event_name == 'schedule'`, NEVER on push/PR) opens a GitHub issue on failure. The VPS gate `deploy/scripts/postdeploy_check.sh` (promtool + repo↔live rule diff — now origin/main blob ↔ live, #1564 — + image-SHA/digest) was already shipped + wired into the `django-prod` runbook in 1a/#362 — Phase 5 adds only the `just` pointer to it. All 4 probes verified green against live vendors before merge. |
