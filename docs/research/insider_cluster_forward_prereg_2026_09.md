...
- **Accrual start** = the first VPS brief date produced with `ALPHALENS_EVENT_LANE=1`, recorded in the ledger entry `outcome.accrual_start` when the flag is flipped (after #1297 is deployed, never before). Rows before that date do not exist. **Recorded 2026-09-06: accrual start = brief date 2026-09-05** (flag set after #1307 + #1340 were deployed; first run surfaced ENOV and EQPT, arrival session 2026-09-08).
...
Logged under §10. Nothing above this section is rewritten.

### D1 (2026-09-11, #1416): the ladder anchor moves to the reader session

**What changes.** The population monitor's ladder replay (entry fills, TTL, time stop, `realized_r`) now starts at `session_on_or_after(D + 1 calendar day)`, the first session after brief `D` exists. Before, it started at `session_on_or_after(D)`. The reason is a measured look-ahead in the monitor, which applies to every lane: brief `D` is built on day `D+1` from session `D`'s close, yet the replay filled orders during session `D`. So the clauses in §4 that say the ladder anchor and the event anchor "coincide" and are "structurally identical" are no longer true.

**Why this is not a §3-§8 change.** Every frozen definition in §4 is unchanged: acceptance time, arrival session, the brief-date mapping, the `car_*_event` anchor price (the arrival-session OPEN), and the `car_*_reader` anchor. Every estimand, floor and rule in §6-§8 is unchanged. The replaced clause describes the population monitor, which is not a registered quantity. The ladder quantities are listed only in §9 ("Required descriptive outputs (frozen list; none is a test)"): fill rate inside the 7-session TTL and `realized_r` of filled episodes. They are now computed from the reader session, which equals the pre-registered `car_*_reader` anchor.

**State at the deviation date** (store read 2026-09-11 12:47 UTC): 5 event rows, 0 terminal, 0 `car_20_event` values. No aggregate has been computed, so the look is not spent (§10). The §12 argument for keeping the primary anchor at the public-information session still stands for the CAR outcomes. It no longer applies to the ladder.
