...
### §16.A6 — Weighting choice locked: α2 sub-leveraged, N_FIXED=150
- AMENDED 2026-05-14 per B0 cost-model audit (`docs/research/paradigm14_pead_cost_model_audit_2026_05_14.md`).
- RE-AMENDED 2026-05-14 (same day) per zen review §H1: Little's Law gives steady-state L=λW where λ=2000 events/252td≈7.94/day and W=20d, so L_avg≈159 names concurrent (top-quintile ≈32) and L_peak≈100 top-quintile during earnings weeks. Initial N_FIXED=30 was incorrect — see audit memo §5.1 for full re-derivation.
...
- DECISION: **α2 sub-leveraged, N_FIXED=150**. Each active position weight = 1/150, gross ∈ [0, 1]. New entrants add 1/150 without forcing existing positions to rebalance; set-based turnover correctly captures all weight changes.
- N_FIXED RATIONALE: 150 = peak top-quintile concurrent (~100) + 50% safety margin. Off-peak gross ≈ 0.21 (avg); peak ≈ 0.67. Pure α2 mechanics preserved at all concurrency levels ≤ N_FIXED.
