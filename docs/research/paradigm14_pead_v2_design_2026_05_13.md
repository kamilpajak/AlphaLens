...
### §16.A6 — Weighting choice locked: α2 sub-leveraged, N_FIXED=30
- AMENDED 2026-05-14 per B0 cost-model audit (`docs/research/paradigm14_pead_cost_model_audit_2026_05_14.md`).
- VERDICT: cost model charges (a) entry YES, (b) exit YES, (c) forced-rebalancing NO. `metrics.py::turnover_pct` and `per_rebalance_turnover` are set-based (`exits = prev_set − curr_set`), not weight-based. Worst-case undercount factor 2× on gross=1 daily-rebal transitions during peak earnings season.
- DECISION: **α2 sub-leveraged, N_FIXED=30**. Each active position weight = 1/30, gross ∈ [0, 1]. New entrants add 1/30 without forcing existing positions to rebalance; set-based turnover correctly captures all weight changes.
- N_FIXED RATIONALE: 30 calibrated against ~25 announcements/day × 20-day hold ÷ ~500 universe → ~25-35 active concurrent during Feb peak. Off-peak gross ≈ 0.17-0.33; peak ≈ 0.83-1.00.
- TRADE-OFF: forfeits ~30-50% theoretical max Sharpe (capital not always deployed). Price of clean cost accounting; preferred over α1 + ~2 sessions of weight-based turnover engineering + multi-day regression risk on closed-paradigm audit replays.
...
- B1 IMPL IMPLICATION: `score_pead_pss.py` returns daily weight DataFrame with per-position weight = 1/30 (constant), gross variable. Engine adapter (Option α B2) consumes as-is.
