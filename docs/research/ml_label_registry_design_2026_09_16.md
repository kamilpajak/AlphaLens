...
3. **Store both scales:** the raw abnormal return AND the same return divided by ex-ante residual volatility × √h. The primary is the RAW label (D1-D4); the scaled label is only stored beside it — it is not an owner decision, no registered test uses it, and making it the target of a model needs its own decision. ATR is the only standing signal and ATR spans 2.6× across candidates, so which scale the model trains on decides what "ATR predicts" means. No separate ATR test is run on this label; ATR is confirmed once in #1227 and reported beside every model (§8 step 3). §4, §6.
...
6. **Horizons 1 / 3 / 5 / 10 / 20 / 40 stored; primary = the 20-session terminal value `sel_ar_20`** (D1, locked before any outcome read). Secondary, pre-specified: `sel_car_mean_20` (mean of the cumulative path over sessions 1-20) and the path at 1 / 3 / 5 / 10 / 20. Descriptive only: returns by catalyst anchoring (other firms / no firm / unmatched). §12 D1.
...
4. **The benchmark is the cheapest alternative the decision replaces, chosen by what it removes out of sample.** For a thematic small / mid-cap pick the size-matched market (IWM) is the minimum; a sector leg is added only if it removes materially more noise out of sample. Measured (§4): it does not, and the small-cap sector ETFs are too thin, so the primary is IWM alone and sector legs are sensitivities (§5.1, §12 D2).
...
**Catalyst anchoring (descriptive only, briefed rows).** How a name enters the brief, verified in code and on briefs: (1) a THEME is mapped when, over the last 7 days, it appears at least 3× more often than its 30-day daily rate (top 10 themes a day; `themes.roll_up`, `flag_novel`); (2) for each theme ONE catalyst article is resolved — the latest theme-tagged event, or, when it names at least two firms, the earliest event of the same entity story (`catalyst_resolver.find_trigger_event`) — and the mapper proposes tickers from that one article only (`theme_mapper.build_prompt`). On 81 of 83 theme-days with more than one briefed pick, all picks shared one article. The originally planned split "the article names the firm" is empty: on 632 briefed picks (July-September) the article names the pick itself in 0.3 %, other firms in 51 %, no firm in 35 %, and 14 % could not be joined to their extraction (`source_event_url` → `thematic_news.url` → `thematic_events.primary_entities`). It is replaced by `catalyst_anchoring` ∈ {`other_firms`, `no_firm`, `unmatched`}, reported as counts and return distributions only: no test, no look charge, no claim about a mechanism. It is NOT a linked-firm variable (a named mega cap is not a verified customer or supplier; Cohen-Frazzini 2008 and Menzly-Ozbas 2010 use verified links), LLM entity extraction is noisy, unmatched rows are never folded into `no_firm`, and rejected proposals have no catalyst row. A verified same-industry or supply-chain variable would need its own registration.
...
Why the sector labels are still stored: briefs are theme-clustered (a third of episodes in XLK), so market-relative labels carry the theme's common move; the within-theme selection question (#1268 pre-registered `sector_excess_return` as its secondary for exactly this) reads the sector versions. They are sensitivities, not the primary (principle 4). Characteristic-matched controls (DGTW: size, book-to-market, momentum) are the literature's stronger benchmark but need point-in-time shares outstanding and book values for the whole market, which the stores do not carry — deferred, not rejected.
...
- **Registered looks keep their registered outcome.** Cluster families on `car_10`, the options first look on `car_10_ex0`, the insider-cluster lane on `car_20_event`, the experts last look on `car_10`. Switching a registered outcome after registration is a forking path. One exception, used once (§8 step 3): a primary may be changed BEFORE any held-out outcome of that plan has been read, with a dated ledger amendment. #1227 moves clusters 1, 2 and 9 from `car_10` to `sel_ar_20` under this exception; nothing was read.
...
3. **No separate ATR test (D5).** The draft proposed one registered look at ATR on `sel_ar_20`. Rejected: ATR already has a held-out confirmation planned (#1227: ATR, MA50 extension, press gate, Holm, power-gated, Wake 2026-10-14) and a July kill line for the live ATR tilt (`selection_score_v2_ext_tilt_decision_2026_07_06.md` §5), both on the same held-out episodes. A third analysis would be another look at the same signal on the same data (ledger rules 3 and 6). Instead:
   ...
   - **The ATR-tilt kill line is NOT run on its own.** It was registered on 2026-07-06 with a trigger of ≥ 40 fresh `car_10` brief-days plus regime diversity, which is probably met by now; running it separately would spend the held-out episodes the combined #1227 decision needs.
   - **Owner principle (2026-09-17): brief ORDER depends only on the score, and producing the score is the job of future models trained on this registry.** The current `selection_score` (`layer4_weighted_score` minus the ATR penalty, `scorer-v1-atrtilt-lam1.0-lo5.77-hi8.37`) stays frozen — no new hand-made terms — until a model score passes its own registered test.
...
|---|---|---|---|
| D1 | Primary selection horizon | **`sel_ar_20`, the terminal value at session 20.** Secondary, pre-specified: `sel_car_mean_20`, the path at 1 / 3 / 5 / 10 / 20. Descriptive only: catalyst anchoring (other firms / no firm / unmatched). Inference stricter than a plain Newey-West test | synthetic simulation on the real session structure; adversarial review reversed an early preference for the path mean; literature on reaction speed |
...
- **Re-run after D2 (whole-set review).** The first simulation used a Blume-adjusted beta and measured the between-session mean, while D2 locked the raw beta and §5.1 asks selection questions within session. Re-run with the raw beta and a within-session slope on a pick-level feature (effect δ·z·path, z standard normal, session-demeaned), minimum detectable effect per 1 SD of the feature: immediate jump A20 2.2 / C20 1.4; jump half given back A20 4.4 / C20 1.9; steady rise A20 2.2 / C20 2.7; delayed A20 2.2 / C20 3.5; slow to 40 A20 4.4 / C20 5.4 (A40 3.0). Between-session level with the raw beta: A20 2.66 / C20 1.56 on the immediate jump. The ranking and the D1 decision are unchanged.
...
| Benchmark-only window read (IWM / SPY legs only, 359 elapsed episodes; the difference between two benchmark versions of one label contains no stock return) | mean label shift Blume vs β = 1 0.00 %; IWM vs SPY 0.28 %; per-episode shift sd 1.2 % (Blume vs β = 1), 0.6 % (raw vs Blume) |
...
### Whole-set review (2026-09-17)

The five decisions were read together by an independent reviewer (`deepseek/deepseek-v4-pro`, given the owner-facing record and this memo, not the author's conclusions) and checked by running them. Accepted and fixed here: the D1 simulation answered the between-session question under the draft beta (re-run above, decision unchanged); §7 had no exception for a pre-read primary change; the owner reading time ignored the DST offset; principle 4 and the sector paragraph still described the draft; `sel_zar_h` read like an owner decision; the ATR kill line had no explicit ban on a separate run. Found by the author: the "names the firm" split was empty (replaced by descriptive catalyst anchoring, owner decision 2026-09-17). Rejected: a claim that the benchmark-only read needed pick returns (the stock return cancels in the difference) and a rule-count mismatch between the two records (same content).
