...
2. **Selection label `sel_ar_h`:** anchor at the OPEN of the first session a reader of the brief can trade (`ladder_arrival_session`), and only when the date's candidate set was final before that open; fixed horizon `h`; benchmark = IWM times a Blume-adjusted 250-session beta (one liquid factor; out of sample no richer benchmark removed materially more noise, §4); gross of cost; continuous. Sector-relative (large-cap SPDR, small-cap sector ETF) and SPY variants stored beside it as sensitivities, and every registered test is repeated on them. §5.1.
...
**Benchmark.** Primary: IWM times a Blume-adjusted beta, `beta_adj = 0.67 * beta_ols + 0.33`, with `beta_ols` the OLS slope of daily stock returns on daily IWM returns over the 250 sessions ending at close(`t0` − 1); fallback `beta_adj = 1` with a coded `beta_source` when the window is thin or degenerate (the `estimate_beta` contract, #996).

Why this and not the richer model the second pass adopted (kept below as a record of what was rejected): out of sample, inside the pre-anchor window, no benchmark removed materially more residual variance than any other — all sit within 5 % of SPY, and the in-sample winner (IWM + small-cap sector) ranks near the bottom on 20-session blocks (§4). Idiosyncratic risk dominates these names. The choice therefore goes to the factor with the fewest estimated parameters, deep liquidity, and a size match to the $0.5-10 bn candidates: IWM. Blume shrinkage toward 1 needs no peer set, so it cannot borrow information from later episodes (the Vasicek peer-mean design had exactly that look-ahead risk, §11 third pass).
...
- **Construction (kept for the primary).** Daily abnormal returns compounded as two wealth paths (stock, fitted benchmark) from `O[t0]` — not a single cumulative ETF return times a beta. Intercept `a_i` is NOT carried into the label (a 250-session alpha of a news-selected small cap is momentum, not an expected return).
...
```
W_stock[t] = W_stock[t-1] * (1 + r_i[t]);   W_bench[t] = W_bench[t-1] * (1 + beta_adj * r_IWM[t])   (both = 1 at O[t0])
...
| Market-model beta | **taken, one factor (IWM), 250 sessions, Blume-adjusted** | §4: 60-session beta is noise-dominated; out-of-sample fit shows no richer benchmark removes materially more noise (third pass, §11) |
...
3. **One registered look: the ATR interpretation test.** Held-out panel, one charge to cluster 1.
   - **Estimand: the TOTAL effect of ATR** on the within-session abnormal return. MAX and the prior return may lie on the path from volatility to return (a volatile name is the one that printed a MAX day), so controlling for them would block part of the effect being measured.
   - **Primary (one test):** OLS of `sel_ar_20` on standardised pre-event ATR % with session fixed effects and pre-treatment controls only — log market cap, liquidity (zero-volume share), sector. CR2 errors by arrival session, a null-imposed wild cluster bootstrap, and the calendar block bootstrap beside them.
   - **Pre-registered robustness, same look:** the primary re-run on each sensitivity label (`_spdr1`, `_psc1`, `_spy`), and on the full date set INCLUDING the 16 `set_final_after_open` dates. A sign change is reported as fragility.
   ...
   - **Descriptive, not a test:** the same model adding MAX and the prior 20-session return. The change in the ATR coefficient is reported as a mediation DESCRIPTION (how much of the association runs alongside lottery-type days; Ang-Hodrick-Xing-Zhang 2006, Fu 2009, Bali-Cakici-Whitelaw 2011), never as proof that the effect is "volatility, not lottery": with ~73 sessions and a plausible mediator, that decomposition is not identified. Measured VIF is moderate (≤ 2.0), so the limit is identification, not collinearity. No scaling is used to make the finding go away.
...
| D2 | Primary benchmark | SPY β=1 / large-cap SPDR sector / small-cap two-factor / **IWM × Blume-adjusted 250-session beta** | IWM one-factor — out of sample all benchmarks leave 95-100 % of SPY's residual variance, so the simplest liquid, size-matched factor wins; SPDR, small-cap sector and SPY stored and re-tested |
