...
1. **Event drain:** a stage that reads the detector's `seen_events` (already universe-wide) for SC 13D / Form 4 clusters and emits `event_candidates/<date>.parquet` with source + gate fields (PR, TDD, zen).
