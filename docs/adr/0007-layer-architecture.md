...
- **Primary success metric for overlay-bearing strategies:** Sharpe-net-improvement vs ungated BASE, multi-phase mean (robust to time-varying beta).
- **Secondary metric:** Carhart-4F α t-stat (reported but flagged as upper-bound estimate).
- A conditional-beta regressor is on the deferred list.
