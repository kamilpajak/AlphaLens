...
- [ADR 0015 — LIVE order rail: keyed day-bound unlock (attended probes only)](0015-live-order-rail-keyed-unlock.md) — 2026-08-10 — `SaxoClient` accepts a non-SIM base URL iff `ALPHALENS_SAXO_LIVE_ORDERS_UNLOCK` equals today's UTC date; `from_env` stays unconditionally SIM; enabled the attended execution-fidelity probes (native trailing ratchet, narrow-amend, shortfall)
- [ADR 0016 — Per-environment broker state separation (two-instance model)](0016-per-environment-broker-state-separation.md) — 2026-08-10 — instance identity via `ALPHALENS_BROKER_ENVIRONMENT` (sim default); journals/picks/KILL/metrics namespaced per instance behind one `state_paths` seam; layered KILL (instance + global); LIVE instance boot structurally blocked until its client wiring (future ADR 0017)
- [ADR 0017 — Standing-LIVE authorization for the unattended broker-manager daemon](0017-standing-live-authorization.md) — 2026-08-10 — replaces the attended day-bound unlock for the daemon context: constructor capability keyword + separate LIVE factory + account-bound standing grant + safety-rail boot-assert; SIM/default paths stay LIVE-incapable
