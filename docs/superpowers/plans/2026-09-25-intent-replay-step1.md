...
- **PR 5 must produce the gate's `read` set from a RECORDING accessor, never from a hand-typed list.** `intent_replay.classification.check_classified(document, read)` subtracts the listed classes and `read`; the interpreter is the only source of `read`, and a list typed by hand is exactly the trusted list the gate exists to refute. The paths the interpreter must prove by reading are the `interpreted` rows of the three §4.3.1 tables minus the two in `REFUSED_BY_DOOR`; `tests/intent_replay/test_classification.py` reads them off the spec, and it KEEPS doing so — several of its tests need a read set no interpreter can produce (an all-paths document declares both reaction kinds, which `validate_intent` refuses). The interpreter's own read set is held to the same tables in `tests/intent_replay/test_interpreter.py`.
...
- **PR 4 landed, and what it hands on.** `door.admit(document)` returns `Admitted(intent, document)`; `document` is the COMPLETED wire document and is the classification gate's universe for PR 5, never the author's. `meta.trade_date` is stated in the document (spec §6.4), so every test that pushes a template through the door adds one. `test_door.py` asserts that the door ADMITS `immediate-plus-pullback`, and that assertion STAYS: the interpreter is what refuses the file, so PR 5 adds the refusal beside it rather than flipping it. `discarded_paths` and `supplied_derived_paths` now live in `broker_contract.trade_intent.codec` (second use); the arming door imports them.
- **PR 5 landed, and what it hands on.** `interpreter.interpret(intent, document)` returns a `Plan` carrying `entries` (a `PendingEntry` per `pullback` rung: `tier_index`, `limit_price`, and `notional` = that rung's share of `spec.size.notional_acct` in the ACCOUNT currency), `notional`, `declared_floor` (`spec.disaster_stop`), `declared_stop` / `declared_take_profit` (`exit.initial_levels.*`, or `None`), `declared_tranches` (the author's ladder, `fraction` = `tranche_pct / 100`), `reaction` (the DECODED primitive, not a copy) and `read` (the gate's universe). The four steps are: the reading pass, `check_classified`, `entry_mode_unsupported`, the plan — the gate BEFORE the refusal, because §4.3.1 calls it the fifth gate of the door's chain, and the reading pass therefore reads every tier including an `immediate` one. `run` calls it between the door and the configuration parse, and an accepted document still prints nothing.
- **PR 5 deliberately decides NOTHING about which declared level rests at a broker; the spec decided it on 2026-09-26 (#1597).** Established by running both paths on 2026-09-26: on both deployments a document supplying `exit.initial_levels` still rests `spec.disaster_stop`. The `planned` journal line is the one the protection pass places and both stop arms read as the never-below floor and the 1R denominator; on the entry-trail path it is written by `control_loop._journal_entry_planned_disaster` from `record["disaster_stop"]` (`:3012,3026`) off a `watch_open` carrying `float(plan.disaster_stop)` (`:2148`). The geometry-aware writer `_planned_exit_levels` is only on the classic bracket path, which such a document cannot reach: with `ENTRY_TRAIL_BPS` set (both deployments) it is intercepted into the watch, and with the trail off `_refuse_geometry_without_trail` (`:9740`) refuses it. What `initial_levels.stop` does reach is the `tranche_plan` line. So spec §5.1 ("the never-naked cover places THAT") and the §4.3.1 row "the level placed, and the R denominator" are both refuted, and the take-profit half is unaddressed by the spec at all while the daemon is unambiguous (one 100% tranche at `initial_levels.tp`, so an author's declared ladder never fires). That was settled by a decision PR on the spec rather than inside an implementation PR, on #1592's own precedent: spec §5.1 now measures R to `spec.disaster_stop` in every document, and a document supplying `initial_levels` fires one 100% tranche at its `tp`. So PR 6 passes `declared_floor` as the `plan_stop`, and `declared_stop` is carried but not placed. The live-side question — whether the keeper SHOULD place a declared stop — is #1598 and blocks nothing here.
- **PR 7 gets no share quantity and no cash from PR 5.** A rung carries a budget, not shares: the published examples are cross-currency, and the FX rate, the whole-share floor and the currency of the minimum commission are #1592. The arithmetic says the deferral costs nothing — with the drain's formula the rate cancels in the account-currency result — but one difference from the daemon stands until then: a rung whose budget buys less than one share exists in the replay and rests nowhere live.
