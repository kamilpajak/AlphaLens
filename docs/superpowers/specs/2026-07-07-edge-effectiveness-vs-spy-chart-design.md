...
- **`ExcessScatter.svelte`** — new component under `apps/web/src/lib/components/`. Renders: scatter points, a smoother line, a shaded band, and a zero reference line.
  - **Charting-lib decision (SUPERSEDED → LayerChart v2):** Lightweight Charts (the candlestick lib in `LadderChart.svelte`) **cannot** render this scatter — every LWC series requires **unique, ascending timestamps**, and multiple positions routinely mature on the same `matured_at` (many y-values at one x). The MVP first shipped a **hand-rolled inline SVG** (no dependency), but it lacked axes/ticks/gridlines/tooltips and read as unreadable. **Replaced by LayerChart v2** (`layerchart@2.0.1`, Svelte-5-native, built on LayerCake + d3-scale; chosen over Observable Plot/ECharts for full Tailwind control of the terminal theme — see the Perplexity dataviz review). Composition: `<Chart data={points} x="date" y="excess">` → `<Layer>` with `<Axis placement="left|bottom" grid rule>` (real % + date axes, gridlines), `<Area data={trend} y0=lo y1=hi>` (CI band), `<Spline data={trend} y=mean>` (smoother), `<Rule y={0}>` (dashed SPY-parity), `<Points>` (scatter), and `<Tooltip.Context mode="quadtree"/>` + `<Tooltip.Root>` (hover). All statistics still come from the server; the component maps them to marks with **no client-side recompute**. The pure `excessScatter.ts` geometry module and its vitest test were removed (LayerChart owns scales/axes). Verification moves to the Playwright panel smoke + `pnpm check`; the panel still lazy-fetches on expand. `data-testid="excess-scatter"` (root) and `data-testid="excess-scatter-trend"` (the trend `<g>`) are kept for the smoke.
