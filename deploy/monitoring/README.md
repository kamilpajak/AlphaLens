...
**The arrival exclusion is load-bearing.** A date whose arrival session has not closed yet has no
bar to fetch, so its null price paths say nothing about whether the run kept up. That is not a
weekend special case: the nightly runs at 06:30 UTC, before the open, so the newest brief date is
in this state every single morning. The first version of this metric counted them and the alert
fired on its first day (2026-09-20 07:40 UTC) on 30 rows from the Friday and Saturday briefs,
whose arrival was the following Monday.

| Rule | Reads | Fires when |
|---|---|---|
| `AlphalensFeedbackRowsUnpriced` | `alphalens_feedback_unpriced_rows` | two consecutive nightly runs both left plannable rows with no price path, on dates whose arrival session had already closed. One night of deferred work is the design working — the next night is meant to pick it up; failing to heal overnight is the starvation shape, because the sweep runs newest-date-first and the oldest dates sit at the tail of the budget every night. |
