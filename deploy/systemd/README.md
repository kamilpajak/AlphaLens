...
.venv/bin/alphalens broker arm S --date <YYYY-MM-DD> --frame <account-equity> --currency <ACCOUNT_CCY>
...
| Arm a new pick | `.venv/bin/alphalens broker arm TICKER --date YYYY-MM-DD --frame EQUITY --currency CCY` (the brief percent becomes a fixed amount at arm time, #1467; daemon picks it up next tick, joined to `submissions.jsonl` so it places once) (`--env sim\|live` selects the instance inbox; default sim — LIVE twin: §9.4) |
...
.venv/bin/alphalens broker arm TICKER --date YYYY-MM-DD --frame 15000 --currency PLN --env live
journalctl --user -u alphalens-broker-manager-live.service -f
```
