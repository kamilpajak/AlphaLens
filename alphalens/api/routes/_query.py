...
def validate_date(value: str, field_name: str = "date") -> str:
    """Enforce a YYYY-MM-DD shape before letting it into a SQL parameter."""
    if len(value) != 10 or value[4] != "-" or value[7] != "-":
        raise HTTPException(status_code=422, detail=f"{field_name} must be YYYY-MM-DD")
    head, sep1, mid, sep2, tail = (
        value[0:4],
        value[4],
        value[5:7],
        value[7],
    ...
    if not (head.isdigit() and mid.isdigit() and tail.isdigit() and sep1 == "-" and sep2 == "-"):
        raise HTTPException(status_code=422, detail=f"{field_name} must be YYYY-MM-DD")
    return value
