...
Public surface:
- :class:`alphalens.preaudit.profiles.SmokeProfile` — per-strategy
  declarative config (smoke window, args, data deps).
- :class:`alphalens.preaudit.profiles.SMOKE_PROFILES` — registry keyed
  by strategy name (must intersect :data:`alphalens_cli.commands.audit._SCRIPTS`).
- :func:`alphalens.preaudit.coverage.check_all_deps` — validate data
  presence + coverage for a profile.
- :func:`alphalens.preaudit.runner.run_smoke` — execute a tiny
